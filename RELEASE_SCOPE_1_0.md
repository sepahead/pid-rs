...
| pid-core.research.isx-heuristics | `experimental::isx_heuristics` | experimental-heuristics | research-only | formula-labelled heuristic baselines that do not estimate paper-defined continuous shared exclusions / `heuristic-baselines-v1` | `heuristic-baselines-with-integer-harmonic-ksg-v2` | exploratory comparison only | method identity; input and preprocessing identity | invalid shape or row count; resource preflight rejection; checked size overflow | pid_core::experimental::isx_heuristics | experimental.migration.compute_pid2; experimental.migration.compute_redundancy | standalone pid-rs callers; no downstream compatibility claimed | no |
