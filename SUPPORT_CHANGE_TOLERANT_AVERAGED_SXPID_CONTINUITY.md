...
| Lean | Finite-vector overlap, residual, entropy, and transfer lemmas; exact heterogeneous keyed Sx events; the finite equivalence-union load bound and its $A_\beta$, $B_\beta$, and $C$ corollaries; a generic row-sum lemma; scalar endpoint bounds; and, separately, all 24 supplied exact-count two-source informative, misinformative, and signed-net cumulative and concrete Möbius-atom coordinates with exact products and sign and zero equivalences | The support-change logarithmic transfer between two laws; bundled total-variation semantics; the published component-atom nonnegativity theorem; bytes-to-counts; Rust or floating-point refinement; certifier/parser execution; higher-source lattices; and statistical or population validity |
