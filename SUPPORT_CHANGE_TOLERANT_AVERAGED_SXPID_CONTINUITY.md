...
| Lean | Finite-vector overlap, residual, entropy, and transfer lemmas; exact heterogeneous keyed Sx events; the finite equivalence-union load bound and its $A_\beta$, $B_\beta$, and $C$ corollaries; a generic row-sum lemma; scalar endpoint bounds; and, separately, supplied exact natural counts with positive total through the four fixed two-source signed-net cumulative logarithms and positive-support averages | The support-change logarithmic transfer between two laws; bundled total-variation semantics; complete informative/misinformative averaged components; concrete full-lattice atoms and the published sign theorem; bytes-to-counts; Rust or floating-point refinement; certifier/parser execution; and statistical or population validity |
