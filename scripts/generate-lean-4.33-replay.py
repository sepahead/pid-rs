...
PINNED_ROOT = Path("/private/tmp/pid-rs-c9-diagnostic.GIjveb/repo")
...
COMPOSITE_V9_CHECKER_RELATIVE = "scripts/check-ksg-m1a-composite-v9.py"
GIT_FIXED_ARGUMENTS = (
    "-c",
...
def validate_composite_v9_cut_bytes(
    lean_raw: bytes,
    composite_raw: bytes,
    ...
    operational_assignment_pattern = re.compile(
        r'^    "scripts/check-ksg-m1a-composite-v9\.py": .+$', re.MULTILINE
    )
    ...
    if (
        ...
        die("composite-v9 checker digest cuts are not unique final literals")
    if (
    ...
    if (
        ...
        or expected_operational.get(COMPOSITE_V9_CHECKER_RELATIVE) != composite_digest
    ):
        die("composite-v9 checker digest cuts do not bind the exact checker bytes")
...
def validate_composite_v9_cut_state(freeze: types.ModuleType, root: Path) -> None:
    lean_raw = freeze.stable_read(
        root / LEAN_CHECKER_RELATIVE, "pre-replay Lean checker cut state"
    ).raw
    composite_raw = freeze.stable_read(
        root / COMPOSITE_V9_CHECKER_RELATIVE,
        "pre-replay composite-v9 checker cut state",
    ).raw
    validate_composite_v9_cut_bytes(
