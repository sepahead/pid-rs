#!/usr/bin/env python3
"""Capture bounded GitHub observations for the composite-v5 correction lifecycle.

The two capture phases are intentionally disjoint. ``predecessor_failure`` preserves the exact
C4 failure boundary and can never qualify a receipt. ``successor_qualification`` observes a
caller-supplied C5 subject and requires three attempt-1 terminal-success runs. On success the
...
This tool executes checksum-bound primitives from the immutable v4 capture tool. It neither
...
if not (
    ...
    print(
        "ERROR: capture-ksg-m1a-composite-v5.py requires Python 3.11+ -I -S -B",
        file=sys.stderr,
    )
    raise SystemExit(2)
...
SCRIPT_RELATIVE = "scripts/capture-ksg-m1a-composite-v5.py"
V4_RELATIVE = "scripts/capture-ksg-m1a-composite-v4.py"
V4_PATH = ROOT / V4_RELATIVE
V4_SHA256 = "7cf9a6fe57c2a828def8789524069e14a21d35739a5019b4310613c8f44065ef"
V4_SIZE_BYTES = 34_626
REPOSITORY = "sepahead/pid-rs"
PREDECESSOR_COMMIT = "da253576a5f76e99633fff4de5cf1118f967b90d"
PREDECESSOR_TREE = "916245b95f90bd98b8bd37a72e148fb9328d5c52"
PREDECESSOR_RUNS = {
    "predecessor_ci": 32_079_866_560,
    "predecessor_codeql": 32_079_865_482,
    "predecessor_contract": 32_079_866_461,
}
...
def _read_bound_primitive(path: Path) -> bytes:
    try:
        ...
        raise _BootstrapError(f"cannot stat immutable v4 primitive source: {error}") from None
    _bootstrap_require(
        stat.S_ISREG(before.st_mode)
        and before.st_nlink == 1
        and stat.S_IMODE(before.st_mode) == 0o644
        and before.st_size == V4_SIZE_BYTES,
        "immutable v4 primitive source metadata changed",
    )
    try:
        descriptor = os.open(path, os.O_RDONLY | os.O_CLOEXEC | os.O_NOFOLLOW)
    except OSError as error:
        raise _BootstrapError(f"cannot open immutable v4 primitive source: {error}") from None
    try:
        ...
        _bootstrap_require(
            ...
            "opened immutable v4 primitive identity changed",
        )
        chunks: list[bytes] = []
        remaining = opened.st_size
        while remaining:
            chunk = os.read(descriptor, min(remaining, 1024 * 1024))
            _bootstrap_require(chunk != b"", "short immutable v4 primitive read")
            chunks.append(chunk)
            remaining -= len(chunk)
        _bootstrap_require(
            os.read(descriptor, 1) == b"", "immutable v4 primitive source grew during read"
        )
    ...
    _bootstrap_require(
        ...
        "immutable v4 primitive source changed during read",
    )
    raw = b"".join(chunks)
    _bootstrap_require(
        hashlib.sha256(raw).hexdigest() == V4_SHA256,
        "immutable v4 primitive source digest changed",
    )
    return raw


def _load_bound_v4(raw: bytes) -> types.ModuleType:
    module = types.ModuleType("pid_rs_capture_ksg_m1a_composite_v4_primitives")
    module.__file__ = os.fspath(V4_PATH)
    module.__package__ = ""
    code = compile(
        raw,
        os.fspath(V4_PATH),
...
try:
    _V4_RAW = _read_bound_primitive(V4_PATH)
    V4 = _load_bound_v4(_V4_RAW)
except (_BootstrapError, OSError, SyntaxError) as error:
    print(f"ERROR: {error}", file=sys.stderr)
    raise SystemExit(2) from None
except Exception:
    print("ERROR: unexpected immutable-v4 primitive load failure", file=sys.stderr)
    raise SystemExit(2) from None


CaptureError = V4.CaptureError
require = V4.require
sha256 = V4.sha256
canonical_json = V4.canonical_json
retain_capture = V4.retain_capture
strict_json = V4.strict_json
read_regular = V4.read_regular
read_token = V4.read_token
bounded_response = V4.bounded_response
open_without_proxy = V4.open_without_proxy
MAX_CAPTURE_BYTES = V4.MAX_CAPTURE_BYTES
MAX_CAPTURE_BODY_BYTES = V4.MAX_CAPTURE_BODY_BYTES
MAX_CAPTURE_ROWS = V4.MAX_CAPTURE_ROWS
MAX_PAGES = V4.MAX_PAGES
API_ROOT = V4.API_ROOT
...
def _download_class(path: str) -> str | None:
    ...
    if (
        re.fullmatch(
            rf"/repos/{re.escape(REPOSITORY)}/actions/jobs/[0-9]+/logs", path
        )
        is not None
    ):
        return "job_log"
...
def request_once(
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "pid-rs-composite-v5-capture",
    ...
    try:
        ...
        require(
            ...
            and any(parsed.hostname.endswith(suffix) for suffix in REDIRECT_HOST_SUFFIXES)
        ...
        redirected = urllib.request.Request(
            location,
            headers={"User-Agent": "pid-rs-composite-v5-capture"},
            method="GET",
        )
...
def fetch(
    ...
    for attempt in range(1, 4):
        try:
            ...
            V4.time.sleep(0.25 * (2 ** (attempt - 1)))
            continue
        ...
        return (
            {
                "body_base64": V4.base64.b64encode(body).decode("ascii"),
...
def capture_run(
    ...
    repository_id = validate_run_document(
        run, role, run_id, head, expected_conclusion
    )
    retain_capture(captures, row)
    jobs = paged(
...
def expected_successor_artifact_names(role: str, head: str) -> set[str]:
    ...
    return {f"ksg-m1a-composite-v5-static-{head}"}
...
class _FixtureResponse:
    def __init__(self, body: bytes, media_type: str = "application/json") -> None:
        self._stream = V4.io.BytesIO(body)
...
def offline_self_test() -> dict[str, Any]:
    primitive_result = V4.offline_self_test()
    require(
        type(primitive_result) is dict and primitive_result.get("result") == "pass",
        "immutable v4 primitive self-test failed",
    )
    ...
    unsafe_redirects = (
        ...
        (log_path, f"https://fixture.blob.core.windows.net/log?token={token.decode('ascii')}"),
        (
            log_path,
    ...
    for path, location in unsafe_redirects:
        def always_redirect(request: urllib.request.Request, target_url: str = location) -> Any:
    ...
    require(
        validate_run_document(success_run, "successor_ci", 101, head, "success")
        == 17
    ...
    for run, role, run_id, expected_head, conclusion in run_mutations:
        _expect_capture_error(
            lambda fixture=run, fixture_role=role, fixture_id=run_id, fixture_head=expected_head, fixture_conclusion=conclusion: validate_run_document(
                fixture,
                fixture_role,
                fixture_id,
                fixture_head,
    ...
    return {
        "immutable_v4_primitives_sha256": V4_SHA256,
        ...
        "schema": "pid-rs/ksg-rev4-m1a-composite-v5-capture-self-test/v1",
        "v4_primitive_self_test": "pass",
    }
...
def capture_configuration(arguments: argparse.Namespace) -> tuple[
...
def main() -> int:
    ...
    try:
        ...
        current_v4 = read_regular(V4_PATH, V4_SIZE_BYTES, 0o644)
        require(
            current_v4 == _V4_RAW and sha256(current_v4) == V4_SHA256,
            "immutable v4 primitive source changed after bound import",
        )
        ...
        result = {
            ...
            "runs": runs,
            "schema": "pid-rs/ksg-rev4-m1a-composite-hosted-capture/v5",
            "schema_revision": 5,
            "subject": subject,
        }
        require(
