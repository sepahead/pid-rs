...
"""Validate the canonical Pandoc 3.10.2 projection of the expanded results guide.

This source-specific successor checks all thirty heading IDs and levels, five
complete table wrappers, and the established image and preamble framing. The
accepted canonical input is copied without changing its bytes. Pandoc 3.1.3 is
rejected here; its historical normalizer and tests remain available separately.

The inherited legacy template fragments document the earlier compatibility
route; version admission keeps that route inactive in this successor. These
checks do not establish mathematical correctness or PDF/UA conformance.
"""
...
SUPPORTED_PANDOC_VERSIONS = (CANONICAL_PANDOC_VERSION,)
VERSION_PATTERN = re.compile(r"pandoc [0-9]+(?:[.][0-9]+){1,3}(?:[-+][0-9A-Za-z.-]+)?")

EXPECTED_HEADING_COMMANDS = {'1-reading-conventions-and-semantic-firewall': 'section',
 'evidence-labels': 'subsection',
 'five-distinct-lanes': 'subsection',
 'lattice-positions-versus-audit-coordinates': 'subsection',
 '2-result-map': 'section',
 'companion-result-packages': 'subsection',
 '3-categorical-sx-theory': 'section',
 '31-foundational-semantic-audit': 'subsection',
 '32-fixed-finite-alphabet-plug-in-convergence': 'subsection',
 '33-support-change-tolerant-averaged-sx-continuity': 'subsection',
 '4-sampling-and-exact-finite-table-assurance': 'section',
 '41-dependency-color-concentration': 'subsection',
 '42-exact-two-source-categorical-sx-assurance': 'subsection',
 '5-higher-source-numerical-and-continuous-estimator-assurance': 'section',
 '51-sxpid3-source-marginal-factorization-and-bounded-audit': 'subsection',
 '52-represented-binary64-and-quantizer-assurance': 'subsection',
 '53-ksg-positive-integer-harmonic-arithmetic': 'subsection',
 '6-estimator-choice-global-nonclaims-and-further-reading': 'section',
 '61-wibral-line-roadmap-for-high-dimension-and-non-euclidean-geometry': 'subsection',
 'a-proved-population-level-transfer-and-why-it-stops-there': 'subsubsection',
 'ranked-research-program': 'subsubsection',
 'what-would-count-as-a-sound-new-estimator': 'subsubsection',
 'concrete-ecosystem-meaning': 'subsubsection',
 '7-companion-results-for-finite-categorical-mgw-questions': 'section',
 '71-a-row-experiment-whose-mean-approaches-an-mgw-atom': 'subsection',
 '72-finite-horizon-error-before-sampling-uncertainty': 'subsection',
 '73-prefix-derivatives-and-the-score-of-every-row': 'subsection',
 '74-a-support-changing-mi-cusp-with-zero-cell-tangents': 'subsection',
 '75-target-copy-identities-and-conditional-entropy-bounds': 'subsection',
 '76-equal-synergy-different-added-shannon-information': 'subsection'}

EXPECTED_HEADING_IDS = ('1-reading-conventions-and-semantic-firewall',
 'evidence-labels',
 'five-distinct-lanes',
 ...
 '61-wibral-line-roadmap-for-high-dimension-and-non-euclidean-geometry',
 ...
 '71-a-row-experiment-whose-mean-approaches-an-mgw-atom',
 '72-finite-horizon-error-before-sampling-uncertainty',
 '73-prefix-derivatives-and-the-score-of-every-row',
 '74-a-support-changing-mi-cusp-with-zero-cell-tangents',
 '75-target-copy-identities-and-conditional-entropy-bounds',
 '76-equal-synergy-different-added-shannon-information')
...
TOP_LEVEL_HEADING_IDS = frozenset({'4-sampling-and-exact-finite-table-assurance', '7-companion-results-for-finite-categorical-mgw-questions', '1-reading-conventions-and-semantic-firewall', '2-result-map', '6-estimator-choice-global-nonclaims-and-further-reading', '5-higher-source-numerical-and-continuous-estimator-assurance', '3-categorical-sx-theory'})
...
def expected_heading_command(heading_id: str) -> str:
    return EXPECTED_HEADING_COMMANDS[heading_id]
...
def validate_document_projection(text: str, mode: str) -> None:
    ...
    if (text.count(CROSSWALK_FRAME_PREFIX) != 1
            or text.count(CROSSWALK_FRAME_SUFFIX) != 1):
        fail("crosswalk page-frame prefix or suffix count changed")
...
def validate_canonical_heading_shapes(text: str) -> None:
    # This is a fixed Pandoc projection, not a general TeX parser. Inspect the
    # document body so template comments mentioning sectioning commands do not
    # enter its roster. Unknown, extra, or reordered sectioning is rejected.
    body = text.split(DOCUMENT_BEGIN, 1)[1].split(DOCUMENT_END, 1)[0]
    observed_commands = tuple(re.findall(
        r"\\(section|subsection|subsubsection|paragraph|subparagraph|chapter|part)"
        r"(?![A-Za-z@])", body
    ))
    expected_commands = tuple(
        EXPECTED_HEADING_COMMANDS[heading_id] for heading_id in EXPECTED_HEADING_IDS
    )
    if observed_commands != expected_commands:
        fail("canonical sectioning command roster changed")
...
def validate_table_shape(lines: list[str], mode: str) -> None:
    ...
    if (
        len(begin_indices) != 5
        or len(end_indices) != 5
        or text.count("\\begin{longtable}") != 5
        or text.count("\\end{longtable}") != 5
    ):
        fail(
...
def normalize(text: str, pandoc_version: str) -> tuple[str, str]:
    ...
    if pandoc_version not in SUPPORTED_PANDOC_VERSIONS:
        fail(
            "Pandoc version is outside the canonical v4 writer projection: "
            f"{pandoc_version}"
        )
    ...
    if normalized.count(TABLE_WRAPPER) != 5:
