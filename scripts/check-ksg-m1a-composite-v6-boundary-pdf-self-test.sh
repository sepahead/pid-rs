...
copy_fixture() {
  ...
  mkdir -p "$destination/audit/evidence/ksg-publication-authorship-2026-09-23/v6"
  cp "$ROOT/audit/evidence/ksg-publication-authorship-2026-09-23/v6/"* "$destination/audit/evidence/ksg-publication-authorship-2026-09-23/v6/"
...
if [[ "$MODE" == "--exact" ]]; then
  echo "OK: composite-v6 boundary PDF exact/cross positives and ten external hostile fixtures plus nine authorship successor controls passed"
else
  echo "OK: composite-v6 boundary PDF cross-toolchain positive and ten external hostile fixtures plus nine authorship successor controls passed"
fi
