#!/usr/bin/env python3
"""Hostile tests for the 28-page mathematical-results guide hosted raw v4 profile.

Pass the exact retained locally captured v4 fixture as the sole argument.  Every subprocess
...
# Measured local translated Linux v4 fixture; inherited hostile controls unchanged.
...
CHECK_NAME = "Mathematical results guide hosted raw-profile v4 self-test"
ROOT = pathlib.Path(__file__).resolve(strict=True).parent.parent
CHECKER = ROOT / "scripts/check-mathematical-results-guide-pdf-hosted-raw-profile-v4.py"
...
EXPECTED_CHECKER_SHA256 = '7791914449814bf203635ff8734055df19301c72ffcf283a14ce4adc991d80bd'
EXPECTED_HOSTED_FIXTURE_BYTES = 818_418
EXPECTED_HOSTED_FIXTURE_SHA256 = '1b1ec340538111b479bbff4bc6f9c686e92ff14a8624a39b4e9e02b0d46a06ce'
EXPECTED_STRUCTURE_CHECK_SHA256 = '0d350c9d76733e287e3efa6e104a9c96446e5304707c73fd0b9ed67f61a87b47'
...
# measured v4 contract bindings. These are not candidate-derived pins.
EXPECTED_CANONICAL_TARGETS_SHA256 = '181d7810ac7ac05597fe56de9c519fdf05f9c4c8bad778ec6fb6f6402ef2a444'
EXPECTED_CANONICAL_NAVIGATION_SHA256 = '551efb7b3c498bee2a7694e969fc91cedb4522bbf8027594805f1896b5e3c5cf'
EXPECTED_CANONICAL_STRUCTURE_SHA256 = '0e155300d1631f61c6090c5ef2a9fc7a8e67b7a470032eaf4759d9bfa682dc1c'
...
def source_invariant_errors(source: bytes) -> tuple[str, ...]:
    ...
    for name, expected in expected_assignments.items():
        ...
        if (
            observed is None
            or len(observed) != 1
            or type(observed[0]) is not type(expected)
            or observed[0] != expected
        ):
            errors.append(f"{name} is not the sole exact reviewed literal and type")
...
def audit_source_mutations(source: bytes, ledger: Ledger) -> None:
    ...
    require_source_mutation_rejected(
        replace_once(
            source,
            f"EXPECTED_HOSTED_FIXTURE_BYTES = {EXPECTED_HOSTED_FIXTURE_BYTES:_}".encode("ascii"),
            f"EXPECTED_HOSTED_FIXTURE_BYTES = {EXPECTED_HOSTED_FIXTURE_BYTES:_}.0".encode("ascii"),
            "fixture byte pin exact type",
        ),
        "floating-point fixture byte pin",
        ledger,
    )
    require_source_mutation_rejected(
        replace_once(
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(
            ...
            require(
                sha256_bytes(expected_targets) == EXPECTED_CANONICAL_TARGETS_SHA256,
                "fixture target report differs from the independent v4 target observation",
            )
            require(
                sha256_bytes(expected_navigation) == EXPECTED_CANONICAL_NAVIGATION_SHA256
                and fixture_report.structure_sha256 == EXPECTED_CANONICAL_STRUCTURE_SHA256,
                "fixture reports differ from the bound v4 structure/navigation contract",
            )
