...
def run_cases(module) -> int:
    with tempfile.TemporaryDirectory(prefix="pid-rs-actions-worktree-config-") as raw:
        ...
        private_root, private_path = fixture(parent, "owner-private")
        private_path.write_bytes(module.EXPECTED)
        private_path.chmod(0o600)
        expect_rejection(module, private_root, "worktree config mode changed")
        require(
            private_path.read_bytes() == module.EXPECTED
            and stat.S_IMODE(private_path.stat().st_mode) == 0o600,
            "owner-private residue changed",
        )
    ...
    print(
        ...
        "10 hostile filesystem/byte states"
    )
    return 0
