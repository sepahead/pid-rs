...
def semantic_capture_source_validation(hosted: bytes, local: bytes) -> None:
    ...
    values = list(
        ...
        for raw, node, label in (
            ...
            (local, nodes[2], "semantic local capture"),
            (local, nodes[3], "semantic local timestamp parser"),
            (local, nodes[4], "semantic local record validator"),
            (local, nodes[5], "semantic local main"),
        )
    )
    ...
    for index, function_name, label in (
        (3, "parse_timestamp", "live local timestamp parser"),
        (4, "validate_record_value", "live local record validator"),
    ):
...
def narrative_semantic_hostiles() -> int:
    ...
    return len(C9.C9_NARRATIVE_PATHS) + 1 + len(targeted) + len(semantic_tokens) + 2
...
def rejected_replay_hostiles() -> int:
    ...
    original = (C9.REJECTED_C9_R14_SHA256, C9.REJECTED_C9_R14_SIZE_BYTES)
    try:
        C9.REJECTED_C9_R14_SHA256 = C9.sha256(fresh_raw)
        C9.REJECTED_C9_R14_SIZE_BYTES = len(fresh_raw)
        expect_rejection(
            lambda: C9.validate_rejected_r14_exclusion(fresh_raw, fresh),
            "rejected r14 bytes reused as current",
        )
    finally:
        C9.REJECTED_C9_R14_SHA256, C9.REJECTED_C9_R14_SIZE_BYTES = original
    hostile = copy.deepcopy(fresh)
    hostile["prior_replay_preservation_sha256"]["rejected_same_slot"] = (
        C9.REJECTED_C9_R14_SHA256
    )
    expect_rejection(
        lambda: C9.validate_rejected_r14_exclusion(fresh_raw, hostile),
        "rejected r14 entered prior replay map",
    )
    return 2
    ...
    try:
        ...
        require(
            ...
            == {
                ...
                "capture_ast_hostiles_rejected": 72,
                "critical_binding_hostiles_rejected": 55,
                "rebind_difference_guard_hostiles_rejected": 3,
                "justfile_hostiles_rejected": 21,
                "lean_cut_hostiles_rejected": 19,
                "narrative_semantic_hostiles_rejected": 36,
                "policy_hostiles_rejected": 20,
                "rejected_replay_hostiles_rejected": 2,
                "replay_route_hostiles_rejected": 17,
