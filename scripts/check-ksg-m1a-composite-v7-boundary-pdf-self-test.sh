...
copy_fixture() {
  ...
  mkdir -p "$destination/audit/evidence/ksg-publication-authorship-2026-09-23/v7"
  cp "$ROOT/audit/evidence/ksg-publication-authorship-2026-09-23/v7/"* "$destination/audit/evidence/ksg-publication-authorship-2026-09-23/v7/"
...
if [[ "$MODE" == "--exact" ]]; then
  echo "OK: composite-v7 boundary PDF exact/cross positives and nineteen external hostile fixtures plus nine authorship successor controls passed"
else
  echo "OK: composite-v7 boundary PDF cross-toolchain positive and nineteen external hostile fixtures plus nine authorship successor controls passed"
fi
