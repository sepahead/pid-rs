#!/usr/bin/env python3
"""Two fixed MGW research-note publications; discovery is never reference adoption."""
...
NATIVE_SHA256 = "7295bafe18f6f6b275f27b8de3b4e30b44c5a81974d5b2326f0ea49a9fed1aa4"
...
KINDS = ("gradient", "cusp")
SOURCES = {
    "gradient": "audit/research/finite-prefix-mgw-gradient/EXPOSITION.md",
    "cusp": "audit/research/support-change-mi-cusp/EXPOSITION.md",
}
OUTPUTS = {"gradient": "finite-prefix-mgw-gradient", "cusp": "support-change-mi-cusp"}
TITLES = {
...
FIGURE = "audit/research/finite-prefix-mgw-gradient/figures/prefix-score-experiment.svg"
STAGE_COMMON = {
    "layout/report.latex": ASSETS + "layout/report.latex",
...
INPUTS = frozenset((*SOURCES.values(), *STAGE_COMMON.values(), FIGURE, NATIVE, READER, RUNTIME))
TOOLS = frozenset(("pandoc", "lualatex", "rsvg-convert", "kpsewhich", "fc-cache",
                   "bash", "pdfinfo", "pdffonts", "pdftotext"))
FONT_NAMES = frozenset(("SourceSansPro-Regular.otf", "SourceSansPro-Semibold.otf",
                       "SourceSansPro-Bold.otf", "latinmodern-math.otf", "lmroman10-regular.otf"))
# The fixed command list below is 20 commands per gradient build, 18 per cusp build.
COMMANDS = {"gradient": 40, "cusp": 36}
...
LOCAL_LINKS = {
    ...
    "../finite-prefix-mgw-gradient/EXPOSITION.md": SOURCES["gradient"],
    "../support-change-mi-cusp/EXPOSITION.md": SOURCES["cusp"],
    "../../../PID_SENSOR_PLACEMENT_AND_GALADRIEL_GUIDE.md#9-ten-grounded-use-cases":
        "PID_SENSOR_PLACEMENT_AND_GALADRIEL_GUIDE.md#9-ten-grounded-use-cases",
...
EXTERNAL_LINKS = frozenset((
    ...
    "https://proceedings.neurips.cc/paper/2015/file/de03beffeed9da5f3639a621bcab5dd4-Paper.pdf",
))
...
def admit_manifest(value: object, kind: str, exact: bool) -> dict:
    ...
    require(type(value) is dict and set(value) == {"schema", "files", "outputs"}
            and value["schema"] == "pid-rs/mgw-derivative-note-inputs-v1",
    ...
    for selected in KINDS:
        ...
        if output["status"] == "unbound":
            ...
            require((selected == "gradient" and type(output["figure_pdf"]) is dict)
                    or (selected == "cusp" and output["figure_pdf"] is None),
...
def admit_registration(reg: object, binding: dict, observed: dt.datetime,
    ...
    require(reg["schema"] == "pid-rs/mgw-derivative-note-execution-v1"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.cross_toolchain:
        print("MGW research-note PDF: no admitted cross-toolchain relation; status 2 refusal.",
              file=sys.stderr)
        return 2
    ...
    if args.exact:
        ...
        if args.kind == "gradient":
            reference_figure = snapshot.read(args.root / (ASSETS + "figures/prefix-score-experiment.pdf"))
            check_entry(reference_figure, expected["figure_pdf"], "canonical vector derivative")
    ...
    try:
        ...
        for number in (1, 2):
            ...
            if args.kind == "gradient":
                stage["figures/prefix-score-experiment.svg"] = FIGURE
            ...
            figure = None
            if args.kind == "gradient":
                # Source uses explicit SVG tspans; no hidden glyph-rewriting stage.
                run("rsvg-convert", ["--format=pdf", "--keep-aspect-ratio",
                    "--output=figures/prefix-score-experiment.pdf",
                    "figures/prefix-score-experiment.svg"], "svg-experiment")
                figure = snapshot.read(build / "figures/prefix-score-experiment.pdf")
                if args.exact:
                    check_entry(figure, expected["figure_pdf"], "figure PDF")
                    require(figure == reference_figure, "vector derivative bytes differ")
            ...
            result["builds"].append({"index": number, "tex": {"bytes": len(tex), "sha256": sha(tex)},
                ...
                "figure_pdf": None if figure is None else {"bytes": len(figure), "sha256": sha(figure)}})
        ...
        require(result["builds"][0]["tex"] == result["builds"][1]["tex"]
                and result["builds"][0]["figure_pdf"] == result["builds"][1]["figure_pdf"],
    ...
    print("MGW research-note PDF: " + result["status"])
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("MGW research-note PDF failed: " + str(error), file=sys.stderr)
        raise SystemExit(1)
