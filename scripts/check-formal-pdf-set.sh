...
if [[ "$MODE" == "--exact" ]]; then
  : "${PID_RS_DERIVATIVE_PYTHON:?formal PDF set: PID_RS_DERIVATIVE_PYTHON is required}"
  : "${PID_RS_DERIVATIVE_MANIFEST_SHA256:?formal PDF set: PID_RS_DERIVATIVE_MANIFEST_SHA256 is required}"
  : "${PID_RS_DERIVATIVE_CONTROL_NORMAL_REGISTRATION:?formal PDF set: PID_RS_DERIVATIVE_CONTROL_NORMAL_REGISTRATION is required}"
  : "${PID_RS_DERIVATIVE_CONTROL_NORMAL_REGISTRATION_SHA256:?formal PDF set: PID_RS_DERIVATIVE_CONTROL_NORMAL_REGISTRATION_SHA256 is required}"
  : "${PID_RS_DERIVATIVE_CONTROL_NORMAL_OUTPUT:?formal PDF set: PID_RS_DERIVATIVE_CONTROL_NORMAL_OUTPUT is required}"
  : "${PID_RS_DERIVATIVE_CONTROL_OPTIMIZED_REGISTRATION:?formal PDF set: PID_RS_DERIVATIVE_CONTROL_OPTIMIZED_REGISTRATION is required}"
  : "${PID_RS_DERIVATIVE_CONTROL_OPTIMIZED_REGISTRATION_SHA256:?formal PDF set: PID_RS_DERIVATIVE_CONTROL_OPTIMIZED_REGISTRATION_SHA256 is required}"
  : "${PID_RS_DERIVATIVE_CONTROL_OPTIMIZED_OUTPUT:?formal PDF set: PID_RS_DERIVATIVE_CONTROL_OPTIMIZED_OUTPUT is required}"
  : "${PID_RS_DERIVATIVE_CONTROL_MONOTONIC_DEADLINE:?formal PDF set: PID_RS_DERIVATIVE_CONTROL_MONOTONIC_DEADLINE is required}"
  : "${PID_RS_DERIVATIVE_GRADIENT_REGISTRATION:?formal PDF set: PID_RS_DERIVATIVE_GRADIENT_REGISTRATION is required}"
  : "${PID_RS_DERIVATIVE_GRADIENT_REGISTRATION_SHA256:?formal PDF set: PID_RS_DERIVATIVE_GRADIENT_REGISTRATION_SHA256 is required}"
  : "${PID_RS_DERIVATIVE_GRADIENT_WORK_DIR:?formal PDF set: PID_RS_DERIVATIVE_GRADIENT_WORK_DIR is required}"
  : "${PID_RS_DERIVATIVE_CUSP_REGISTRATION:?formal PDF set: PID_RS_DERIVATIVE_CUSP_REGISTRATION is required}"
  : "${PID_RS_DERIVATIVE_CUSP_REGISTRATION_SHA256:?formal PDF set: PID_RS_DERIVATIVE_CUSP_REGISTRATION_SHA256 is required}"
  : "${PID_RS_DERIVATIVE_CUSP_WORK_DIR:?formal PDF set: PID_RS_DERIVATIVE_CUSP_WORK_DIR is required}"
  : "${PID_RS_DERIVATIVE_PRODUCTION_MONOTONIC_DEADLINE:?formal PDF set: PID_RS_DERIVATIVE_PRODUCTION_MONOTONIC_DEADLINE is required}"
fi
# END DERIVATIVE PREFLIGHT
...
# BEGIN DERIVATIVE EXACT DISPATCH
# Use original caller registrations; run before long aggregate gates.
if [[ "$MODE" == "--exact" ]]; then
  "$PID_RS_DERIVATIVE_PYTHON" -I -S -B scripts/check-mgw-derivative-notes-pdf-self-test.py --root "$ROOT" --registration "$PID_RS_DERIVATIVE_CONTROL_NORMAL_REGISTRATION" --registration-sha256 "$PID_RS_DERIVATIVE_CONTROL_NORMAL_REGISTRATION_SHA256" --output "$PID_RS_DERIVATIVE_CONTROL_NORMAL_OUTPUT" --stage-monotonic-deadline "$PID_RS_DERIVATIVE_CONTROL_MONOTONIC_DEADLINE"
  "$PID_RS_DERIVATIVE_PYTHON" -O -I -S -B scripts/check-mgw-derivative-notes-pdf-self-test.py --root "$ROOT" --registration "$PID_RS_DERIVATIVE_CONTROL_OPTIMIZED_REGISTRATION" --registration-sha256 "$PID_RS_DERIVATIVE_CONTROL_OPTIMIZED_REGISTRATION_SHA256" --output "$PID_RS_DERIVATIVE_CONTROL_OPTIMIZED_OUTPUT" --stage-monotonic-deadline "$PID_RS_DERIVATIVE_CONTROL_MONOTONIC_DEADLINE"
  "$PID_RS_DERIVATIVE_PYTHON" -I -S -B scripts/build-mgw-derivative-notes-pdf.py --kind gradient --exact --check --root "$ROOT" --manifest-sha256 "$PID_RS_DERIVATIVE_MANIFEST_SHA256" --registration "$PID_RS_DERIVATIVE_GRADIENT_REGISTRATION" --registration-sha256 "$PID_RS_DERIVATIVE_GRADIENT_REGISTRATION_SHA256" --work-dir "$PID_RS_DERIVATIVE_GRADIENT_WORK_DIR" --stage-monotonic-deadline "$PID_RS_DERIVATIVE_PRODUCTION_MONOTONIC_DEADLINE"
...
# BEGIN DERIVATIVE CROSS REFUSAL
if [[ "$MODE" == "--cross-toolchain" ]]; then
  for DERIVATIVE_KIND in gradient cusp; do
    if python3 -I -S -B scripts/build-mgw-derivative-notes-pdf.py --kind "$DERIVATIVE_KIND" --cross-toolchain; then
      echo "formal PDF set: $DERIVATIVE_KIND derivative-note cross-toolchain mode unexpectedly accepted" >&2
      exit 1
    else
      DERIVATIVE_CROSS_STATUS=$?
    fi
    if [[ "$DERIVATIVE_CROSS_STATUS" -ne 2 ]]; then
      echo "formal PDF set: $DERIVATIVE_KIND derivative-note refusal returned $DERIVATIVE_CROSS_STATUS, expected 2" >&2
      exit 1
    fi
  done
fi
