...
V4_MEASUREMENTS = {'role_map_entries': 39, 'id_tree_leaves': 26, 'destination_root_kids': 3, 'destination_leaf_pairs': [32, 32, 16], 'page_count': 28, 'page_tree_kids': 3, 'page_group_counts': [10, 10, 8], 'total_links': 147, 'external_target_count': 75, 'parent_tree_mappings': 164, 'action_owners': 174, 'rotated_page': 7}
...
EXPECTED_NAVIGATION_SHA256 = '551efb7b3c498bee2a7694e969fc91cedb4522bbf8027594805f1896b5e3c5cf'
EXPECTED_STRUCTURE_ELEMENTS = 1273
# Exact normalized tagged-structure manifest for this canonical source profile.
EXPECTED_STRUCTURE_SHA256 = '0e155300d1631f61c6090c5ef2a9fc7a8e67b7a470032eaf4759d9bfa682dc1c'
...
def validate_structure_tree(reader: PdfReader, root: DictionaryObject) -> StructureIndex:
    ...
    if len(role_map) != V4_MEASUREMENTS["role_map_entries"]:
    ...
    if not isinstance(kids, ArrayObject) or len(kids) != V4_MEASUREMENTS["id_tree_leaves"]:
...
def validate_named_destinations(
    ...
    def visit(
        ...
        if expected_leaf_count is not None:
            ...
            if not isinstance(kids, ArrayObject) or len(kids) != V4_MEASUREMENTS["destination_root_kids"]:
                fail("name_tree", f"{path}: expected the bound v4 destination leaves")
            result = []
            for index, leaf_count in enumerate(V4_MEASUREMENTS["destination_leaf_pairs"]):
...
def validate_page_tree(reader: PdfReader, root: DictionaryObject) -> None:
    ...
    require_integer(dictionary_raw(pages, "/Count"), V4_MEASUREMENTS["page_count"], "catalog Pages /Count")
    kids = require_array(dictionary_raw(pages, "/Kids"), V4_MEASUREMENTS["page_tree_kids"], "catalog Pages /Kids")
    expected_group_counts = V4_MEASUREMENTS["page_group_counts"]
    expected_page = 0
...
def validate_pages_and_links(
    ...
    if len(reader.pages) != V4_MEASUREMENTS["page_count"]:
    ...
    for page_number, page in enumerate(reader.pages, 1):
        allowed_page_keys = (
            ...
            | ({"/Rotate"} if page_number == V4_MEASUREMENTS["rotated_page"] else set())
        )
        ...
        rotation = "none"
        if page_number == V4_MEASUREMENTS["rotated_page"]:
    ...
    if total_links != V4_MEASUREMENTS["total_links"] or counts != EXPECTED_LINK_COUNTS:
        fail("link_count", f"link action counts changed: {dict(counts)}")
    if len(targets) != V4_MEASUREMENTS["external_target_count"]:
...
def validate_parent_tree(
    ...
    *,
    ...
    if set(mappings) != expected_keys or len(mappings) != V4_MEASUREMENTS["parent_tree_mappings"]:
...
def validate_action_owners(
    ...
    if len(owners) != V4_MEASUREMENTS["action_owners"]:
