#!/usr/bin/env python3
"""Capture one bounded, fail-closed local composite-v8 closure observation.

The fixed command is ``just ksg-composite-v8``. Version and small metadata
...
if not (
    ...
    print(
        "ERROR: capture-ksg-m1a-composite-v8-local-closure.py requires Python 3.11+ -I -S -B",
        file=sys.stderr,
    )
    raise SystemExit(2)
...
V7_RELATIVE = "scripts/capture-ksg-m1a-composite-v7-local-closure.py"
V7_PATH = ROOT / V7_RELATIVE
V7_SHA256 = "5268322d756e546f29ed9d2ded58264800808570be286adfba562f7decb284c1"
V7_SIZE_BYTES = 35_401
SCRIPT_RELATIVE = "scripts/capture-ksg-m1a-composite-v8-local-closure.py"
SCHEMA_RELATIVE = "audit/schemas/ksg-rev4-m1a-composite-local-closure-v8.schema.json"
...
COMMAND_ARGV = ("just", "ksg-composite-v8")
...
AUTHORITY_ROLES = {
    ...
    SCHEMA_RELATIVE: "local_l8_closure_schema",
    SCRIPT_RELATIVE: "bounded_local_l8_closure_capture_tool",
...
NONIMPLICATIONS = [
    ...
    "One local execution is correlated with the C8 checkout and is neither independent replication nor first-attempt authority.",
...
class BootstrapError(RuntimeError):
    """The immutable v7 local-recorder primitives could not be loaded."""
...
def read_bound_v7() -> bytes:
    ...
    bootstrap_require(
        ...
        "immutable v7 local-recorder primitive metadata changed",
    )
    ...
    try:
        ...
        bootstrap_require(
            ...
            "opened immutable v7 local-recorder identity changed",
        )
        chunks: list[bytes] = []
        remaining = opened.st_size
        while remaining:
            chunk = os.read(descriptor, min(remaining, 1024 * 1024))
            bootstrap_require(chunk != b"", "short immutable v7 local-recorder read")
            chunks.append(chunk)
            remaining -= len(chunk)
        bootstrap_require(os.read(descriptor, 1) == b"", "immutable v7 local recorder grew")
    ...
    for field in (
        ...
        bootstrap_require(
            ...
            == getattr(after, field),
            "immutable v7 local-recorder primitive changed while read",
        )
    raw = b"".join(chunks)
    bootstrap_require(
        hashlib.sha256(raw).hexdigest() == V7_SHA256,
        "immutable v7 local-recorder primitive digest changed",
    )
    return raw


def load_bound_v7(raw: bytes) -> types.ModuleType:
    module = types.ModuleType("pid_rs_capture_ksg_m1a_composite_v7_local_primitives")
...
try:
    ...
    print("ERROR: unexpected immutable-v7 local-recorder load failure", file=sys.stderr)
    raise SystemExit(2) from None
...
try:
    ...
    bootstrap_require(
        ...
        "immutable v7 local-recorder self-test failed before the v8 rebind",
    )
except Exception:
    print("ERROR: immutable-v7 local-recorder self-test failed before v8 rebind", file=sys.stderr)
    raise SystemExit(2) from None
...
# v8 subject. The record construction and authority-bound selection stay local
# to this module and are not monkey-patched into the immutable source bytes.
PRIMITIVES.C5_COMMIT = C7_COMMIT
PRIMITIVES.C6_MESSAGE = C8_MESSAGE
...
def authority_descriptors(
    ...
    for relative, role in sorted(AUTHORITY_ROLES.items()):
        ...
        require(raw == committed, "local authority differs from the C8 tree")
...
def validate_record_value(value: Any) -> None:
    require(
        ...
        and value["schema"] == "pid-rs/ksg-rev4-m1a-composite-local-closure/v3"
        and value["schema_revision"] == 3
        and value["repository"] == REPOSITORY
        and value["limits"] == LIMITS
        and value["nonimplications"] == NONIMPLICATIONS
        and value["immutable_v7_primitives"]
        == {"path": V7_RELATIVE, "sha256": V7_SHA256, "size_bytes": V7_SIZE_BYTES},
        "local closure root identity changed",
    )
    subject = value["subject"]
    require(
        type(subject) is dict
        and set(subject) == {"c7_parent", "c8_commit", "c8_message", "c8_tree"}
        and subject["c7_parent"] == C7_COMMIT
        and type(subject["c8_commit"]) is str
        and SHA1_RE.fullmatch(subject["c8_commit"]) is not None
        and subject["c8_message"] == C8_MESSAGE
        and type(subject["c8_tree"]) is str
        and SHA1_RE.fullmatch(subject["c8_tree"]) is not None,
    ...
    for label in ("before", "after"):
        PRIMITIVES.validate_snapshot_value(
            state[label], subject["c8_commit"], subject["c8_tree"], label
        )
    require(
    ...
    stdout = PRIMITIVES.decode_binding(invocation["stdout"], "command stdout", MAX_COMMAND_STREAM_BYTES)
    stderr = PRIMITIVES.decode_binding(invocation["stderr"], "command stderr", MAX_COMMAND_STREAM_BYTES)
...
def authority_roster_self_test() -> int:
    ...
    expect_capture_error(
        lambda: validate_authority_roster(missing_named_oversize),
        "named authority no longer oversized",
    )
    hostiles += 1
    return hostiles
...
def authority_route_self_test() -> dict[str, int]:
    ...
    original_v7_root = PRIMITIVES.ROOT
    original_v7_runner = PRIMITIVES.run_bounded
    original_v7_version_bound = PRIMITIVES.MAX_VERSION_STREAM_BYTES
    hostiles = 0
    with tempfile.TemporaryDirectory(prefix="pid-rs-c8-authority-route-") as temp_text:
        ...
        environment, _route_digest = PRIMITIVES.minimal_environment(directories, temporary_root)
        git_path = PRIMITIVES.resolve_executable("git", directories)

        def git_fixture(*arguments: str) -> bytes:
            code, stdout, stderr, timed_out = original_v7_runner(
                ("git", *arguments),
                git_path,
                environment,
                repository_root,
        ...
        try:
            ...
            expect_capture_error(
                lambda: PRIMITIVES.git_output(git_path, environment, "show", f"{head}:authority.bin"),
                "generic v7 Git output accepted a 65,537-byte authority",
            )
            hostiles += 1
            ...
            nonzero_result = original_v7_runner(
            ...
            stderr_result = original_v7_runner(
                ("git", "show", f"{head}:authority.bin"),
                stderr_git,
                environment,
                repository_root,
            ...
            PRIMITIVES.run_bounded = original_v7_runner
            PRIMITIVES.ROOT = original_v7_root
            ROOT = original_root
            AUTHORITY_MODES = original_modes
            AUTHORITY_ROLES = original_roles

    require(
        PRIMITIVES.MAX_VERSION_STREAM_BYTES == original_v7_version_bound == MAX_VERSION_STREAM_BYTES,
        "immutable v7 version bound was mutated",
    )
...
def offline_self_test() -> dict[str, Any]:
    require(
        V7_SELF_TEST_BASELINE.get("result") == "pass",
        "immutable v7 local primitive self-test failed",
    )
    require(
        set(AUTHORITY_MODES) == set(AUTHORITY_ROLES)
        and {
            path for path, mode in AUTHORITY_MODES.items() if mode == 0o755
        }
        == {
            FORMAL_PDF_SET_RELATIVE,
            WORKFLOW_PDF_GATE_RELATIVE,
    ...
    for relative, expected_size, expected_sha, expected_oversize in (
        (V7_SELF_TEST_RELATIVE, 58_800, "da29201919922914e138b766a8c7e911d1227c6b21785d0efd41c46a5559bb63", False),
        (V7_CHECKER_RELATIVE, 166_557, "7b9ac4f2e050594ca4f51d8c9609708d714e4a2fec80b1ac17e3b04df53b4993", True),
    ):
        raw = PRIMITIVES.read_regular(ROOT / relative, MAX_AUTHORITY_STREAM_BYTES, 0o644)
        require(
            len(raw) == expected_size
            and sha256(raw) == expected_sha
            and (len(raw) > MAX_VERSION_STREAM_BYTES) is expected_oversize,
            "retained oversized v7 authority changed",
        )
    ...
    return {
        ...
        "immutable_v7_local_self_test": "pass",
        ...
        "schema": "pid-rs/ksg-rev4-m1a-composite-v8-local-closure-capture-self-test/v1",
...
def capture_under_fixed_umask(output_path: str) -> None:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(
            prefix="pid-rs-c8-local-closure-",
            ...
            value = {
                ...
                "schema": "pid-rs/ksg-rev4-m1a-composite-local-closure/v3",
                "schema_revision": 3,
                "subject": {
                    "c7_parent": C7_COMMIT,
                    "c8_commit": before_state["head"],
                    "c8_message": C8_MESSAGE,
                    "c8_tree": before_state["tree"],
                },
            }
...
def main() -> int:
    ...
    try:
        if arguments.self_test:
            ...
            PRIMITIVES.under_fixed_umask(lambda: capture_under_fixed_umask(arguments.output))
        return 0
    except (CaptureError, OSError, subprocess.SubprocessError):
