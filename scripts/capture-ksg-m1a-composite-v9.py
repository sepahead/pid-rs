#!/usr/bin/env python3
"""Capture bounded GitHub observations for the composite-v9 lifecycle.

``predecessor_failure`` preserves exact C8 attempt-1 observations and can never
qualify R8. ``successor_qualification`` requires three fresh attempt-1 terminal
success runs for one caller-supplied C9 subject. The implementation reuses only
...
if not (
    ...
    print(
        "ERROR: capture-ksg-m1a-composite-v9.py requires GIL-enabled CPython 3.14.6 -I -S -B",
        file=sys.stderr,
    )
    raise SystemExit(2)
...
SCRIPT_RELATIVE = "scripts/capture-ksg-m1a-composite-v9.py"
...
PREDECESSOR_RUNS = {
    ...
    "predecessor_codeql": 32_286_212_997,
    "predecessor_contract": 32_286_213_774,
}
...
try:
    ...
    bootstrap_require(
        ...
        "immutable v8 capture self-test failed before the v9 rebind",
    )
except Exception:
    print(
        "ERROR: immutable-v8 capture self-test failed before v9 rebind", file=sys.stderr
    )
    raise SystemExit(2) from None
...
def expected_successor_artifact_names(role: str, head: str) -> set[str]:
    ...
    return {f"ksg-m1a-composite-v9-static-{head}"}
...
def capture_configuration(
    ...
    if arguments.phase == "predecessor_failure":
        ...
        require(
            ...
            "terminal C8 predecessor run roster is unresolved",
        )
        require(
            all(
            ...
            "terminal C8 predecessor conclusions are unresolved",
        )
        return (
            dict(PREDECESSOR_RUNS),
            dict(PREDECESSOR_CONCLUSIONS),
            {role: C8_COMMIT for role in PREDECESSOR_RUNS},
            {"predecessor_commit": C8_COMMIT, "predecessor_tree": C8_TREE},
        )
    ...
    return (
        ...
        {
            "predecessor_commit": C8_COMMIT,
            "predecessor_tree": C8_TREE,
...
def offline_self_test() -> dict[str, Any]:
    ...
    require(
        ...
        and set(predecessor_heads.values()) == {C8_COMMIT}
    ...
    stale_artifact = {
        "expired": False,
        "id": 201,
        "name": f"ksg-m1a-composite-v8-static-{head}",
        "size_in_bytes": 1,
        "workflow_run": {
            "head_branch": "main",
    ...
    for role in exact_artifact_sets:
        ...
        hostile["name"] = (
            f"ksg-m1a-composite-v8-static-{head}"
    ...
    return {
        ...
        "result": "pass",
        "schema": "pid-rs/ksg-rev4-m1a-composite-v9-capture-self-test/v1",
...
def main() -> int:
    ...
    try:
        ...
        if arguments.phase == "predecessor_failure":
            require(
                failed_by_role["predecessor_ci"] == [C8_CI_FAILED_JOB]
                and failed_by_role["predecessor_codeql"] == []
                and failed_by_role["predecessor_contract"] == [C8_CONTRACT_FAILED_JOB],
        ...
        result = {
            ...
            "runs": runs,
            "schema": "pid-rs/ksg-rev4-m1a-composite-hosted-capture/v9",
            "schema_revision": 9,
            "subject": subject,
        }
        require(
