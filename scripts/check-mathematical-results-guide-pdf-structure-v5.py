...
# Source-specific v4 bindings measured from canonical PDF 434fae95216282dc08840a1d492076d5c648d64b01bc1cd939884555b0a615a4.
...
V5_MEASUREMENTS = {'role_map_entries': 39, 'id_tree_leaves': 26, 'destination_root_kids': 3, 'destination_leaf_pairs': [32, 32, 16], 'page_count': 28, 'page_tree_kids': 3, 'page_group_counts': [10, 10, 8], 'total_links': 147, 'external_target_count': 75, 'parent_tree_mappings': 164, 'action_owners': 174, 'rotated_page': 7}
...
EXPECTED_NAMED_DESTINATIONS = {'Doc-Start': 0, 'Item.1': 22, 'Item.10': 23, 'Item.11': 23, 'Item.12': 23, 'Item.13': 23, 'Item.14': 23, 'Item.2': 22, 'Item.3': 22, 'Item.4': 22, 'Item.5': 22, 'Item.6': 23, 'Item.7': 23, 'Item.8': 23, 'Item.9': 23, 'figure.caption.23': 20, 'figure.caption.4': 3, 'figure.caption.7': 6, 'none.1': 2, 'none.2': 2, 'none.3': 5, 'none.4': 7, 'none.5': 17, 'page.1': 1, 'page.10': 10, 'page.11': 11, 'page.12': 12, 'page.13': 13, 'page.14': 14, 'page.15': 15, 'page.16': 16, 'page.17': 17, 'page.18': 18, 'page.19': 19, 'page.2': 2, 'page.20': 20, 'page.21': 21, 'page.22': 22, 'page.23': 23, 'page.24': 24, 'page.25': 25, 'page.26': 26, 'page.27': 27, 'page.3': 3, 'page.4': 4, 'page.5': 5, 'page.6': 6, 'page.7': 7, 'page.8': 8, 'page.9': 9, 'section*.1': 2, 'section*.13': 11, 'section*.16': 13, 'section*.20': 17, 'section*.27': 25, 'section*.6': 5, 'section*.9': 8, 'subsection*.10': 8, 'subsection*.11': 8, 'subsection*.12': 10, 'subsection*.14': 11, 'subsection*.15': 12, 'subsection*.17': 13, 'subsection*.18': 14, 'subsection*.19': 16, 'subsection*.2': 2, 'subsection*.21': 17, 'subsection*.28': 25, 'subsection*.29': 25, 'subsection*.3': 2, 'subsection*.30': 26, 'subsection*.31': 26, 'subsection*.32': 27, 'subsection*.33': 27, 'subsection*.5': 3, 'subsection*.8': 7, 'subsubsection*.22': 18, 'subsubsection*.24': 22, 'subsubsection*.25': 23, 'subsubsection*.26': 23}
EXPECTED_OUTLINE = [(0, 2, '1. Reading conventions and semantic firewall', 'section*.1'), (1, 2, 'Evidence labels', 'subsection*.2'), (1, 2, 'Five distinct lanes', 'subsection*.3'), (1, 3, 'Lattice positions versus audit coordinates', 'subsection*.5'), (0, 5, '2. Result map', 'section*.6'), (1, 7, 'Companion result packages', 'subsection*.8'), (0, 8, '3. Categorical-Sx theory', 'section*.9'), (1, 8, '3.1 Foundational semantic audit', 'subsection*.10'), (1, 8, '3.2 Fixed finite-alphabet plug-in convergence', 'subsection*.11'), (1, 10, '3.3 Support-change-tolerant averaged-Sx continuity', 'subsection*.12'), (0, 11, '4. Sampling and exact finite-table assurance', 'section*.13'), (1, 11, '4.1 Dependency-color concentration', 'subsection*.14'), (1, 12, '4.2 Exact two-source categorical-Sx assurance', 'subsection*.15'), (0, 13, '5. Higher-source, numerical, and continuous-estimator assurance', 'section*.16'), (1, 13, '5.1 SxPID3 source-marginal factorization and bounded audit', 'subsection*.17'), (1, 14, '5.2 Represented-binary64 and quantizer assurance', 'subsection*.18'), (1, 16, '5.3 KSG positive-integer harmonic arithmetic', 'subsection*.19'), (0, 17, '6. Estimator choice, global nonclaims, and further reading', 'section*.20'), (1, 17, '6.1 Wibral-line roadmap for high dimension and non-Euclidean geometry', 'subsection*.21'), (0, 25, '7. Companion results for finite categorical MGW questions', 'section*.27'), (1, 25, '7.1 A row experiment whose mean approaches an MGW atom', 'subsection*.28'), (1, 25, '7.2 Finite-horizon error before sampling uncertainty', 'subsection*.29'), (1, 26, '7.3 Prefix derivatives and the score of every row', 'subsection*.30'), (1, 26, '7.4 A support-changing MI cusp with zero cell tangents', 'subsection*.31'), (1, 27, '7.5 Target-copy identities and conditional-entropy bounds', 'subsection*.32'), (1, 27, '7.6 Equal synergy, different added Shannon information', 'subsection*.33')]
EXPECTED_ACTION_COUNTS = {'/GoTo': 53, '/URI': 121}
EXPECTED_LINK_COUNTS = {'/GoTo': 26, '/URI': 121}
# Authorship successor: source/footer/byline only; v4 checker remains historical.
# Exact normalized navigation manifest for this canonical source profile.
EXPECTED_NAVIGATION_SHA256 = '88a424f3c51050a2d5af85d0303fee2b1849b1678a559648298fd021c12bce7b'
EXPECTED_STRUCTURE_ELEMENTS = 1273
# Exact normalized tagged-structure manifest for this canonical source profile.
EXPECTED_STRUCTURE_SHA256 = 'd8115d660de1bbfc2972e249429fab3d8ee383a2e981985b2b81ee7cd618f293'
...
def validate_structure_tree(reader: PdfReader, root: DictionaryObject) -> StructureIndex:
    ...
    if len(role_map) != V5_MEASUREMENTS["role_map_entries"]:
    ...
    if not isinstance(kids, ArrayObject) or len(kids) != V5_MEASUREMENTS["id_tree_leaves"]:
        fail("structure_tree", "IDTree root does not have the declared v4 leaf count")
...
def validate_named_destinations(
    ...
    def visit(
        ...
        if expected_leaf_count is not None:
            ...
            if not isinstance(kids, ArrayObject) or len(kids) != V5_MEASUREMENTS["destination_root_kids"]:
                fail("name_tree", f"{path}: expected the bound v4 destination leaves")
            result = []
            for index, leaf_count in enumerate(V5_MEASUREMENTS["destination_leaf_pairs"]):
...
def validate_page_tree(reader: PdfReader, root: DictionaryObject) -> None:
    ...
    require_integer(dictionary_raw(pages, "/Count"), V5_MEASUREMENTS["page_count"], "catalog Pages /Count")
    kids = require_array(dictionary_raw(pages, "/Kids"), V5_MEASUREMENTS["page_tree_kids"], "catalog Pages /Kids")
    expected_group_counts = V5_MEASUREMENTS["page_group_counts"]
    expected_page = 0
...
def validate_pages_and_links(
    ...
    if len(reader.pages) != V5_MEASUREMENTS["page_count"]:
        fail("page_count", f"expected the bound v4 page count, observed {len(reader.pages)}")
    targets: set[str] = set()
    navigation: list[str] = []
    ...
    for page_number, page in enumerate(reader.pages, 1):
        allowed_page_keys = (
            ...
            | ({"/Rotate"} if page_number == V5_MEASUREMENTS["rotated_page"] else set())
        )
        ...
        rotation = "none"
        if page_number == V5_MEASUREMENTS["rotated_page"]:
            require_integer(dictionary_raw(page, "/Rotate"), 90, "bound landscape page /Rotate")
            rotation = "90"
    ...
    if total_links != V5_MEASUREMENTS["total_links"] or counts != EXPECTED_LINK_COUNTS:
        fail("link_count", f"link action counts changed: {dict(counts)}")
    if len(targets) != V5_MEASUREMENTS["external_target_count"]:
        fail("link_count", f"expected the bound v4 external HTTPS target count, observed {len(targets)}")
...
def validate_parent_tree(
    ...
    *,
    ...
    if set(mappings) != expected_keys or len(mappings) != V5_MEASUREMENTS["parent_tree_mappings"]:
...
def validate_action_owners(
    ...
    if len(owners) != V5_MEASUREMENTS["action_owners"]:
        fail("action_count", f"expected the bound v4 explicit action-owner count, observed {len(owners)}")
    edges: dict[
        tuple[tuple[str, int, int] | tuple[str, int], str],
        tuple[tuple[str, int, int] | tuple[str, int], str, str],
    ] = {}
