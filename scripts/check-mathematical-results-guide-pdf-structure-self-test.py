...
CHECKER_PATH = ROOT / "scripts/check-mathematical-results-guide-pdf-structure.py"
...
def mutate_action_alias(writer: PdfWriter) -> None:
    annotations = annotations_with_action(writer, "/GoToR")
    by_target: dict[str, list[DictionaryObject]] = {}
    for annotation in annotations:
        target = str(annotation["/A"]["/F"]["/UF"])
        by_target.setdefault(target, []).append(annotation)
    pair = next((items for items in by_target.values() if len(items) >= 2), None)
    if pair is None:
        raise SystemExit("PDF structure self-test failed: no repeated remote-PDF target")
    pair[1][NameObject("/A")] = CHECKER.dictionary_raw(pair[0], "/A")
...
def mutate_remote_page_type(writer: PdfWriter) -> None:
    annotation = annotations_with_action(writer, "/GoToR")[0]
    annotation["/A"]["/D"][0] = FloatObject(0.9)


def mutate_remote_fit_type(writer: PdfWriter) -> None:
    annotation = annotations_with_action(writer, "/GoToR")[0]
    annotation["/A"]["/D"][1] = TextStringObject("/Fit")
...
def mutate_filespec_type(writer: PdfWriter) -> None:
    annotation = annotations_with_action(writer, "/GoToR")[0]
    annotation["/A"]["/F"][NameObject("/Type")] = TextStringObject("/Filespec")
...
def mutate_filespec_f_encoding(writer: PdfWriter) -> None:
    annotation = annotations_with_action(writer, "/GoToR")[0]
    specification = annotation["/A"]["/F"]
    specification[NameObject("/F")] = utf16_text(str(specification["/F"]))


def mutate_filespec_uf_encoding(writer: PdfWriter) -> None:
    annotation = annotations_with_action(writer, "/GoToR")[0]
    specification = annotation["/A"]["/F"]
...
def main(argv: list[str]) -> int:
    ...
    if len(baseline.targets) != 43 or len(baseline.navigation) != 167:
    ...
    cases = [
        ...
        ("remote-page-type", mutate_remote_page_type, "typed_integer"),
        ("remote-fit-type", mutate_remote_fit_type, "typed_name"),
        ("uri-action-type", mutate_uri_action_type, "typed_name"),
        ("filespec-type", mutate_filespec_type, "typed_name"),
