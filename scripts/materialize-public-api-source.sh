...
if [[ "$#" -ne 4 ]]; then
  echo "usage: $0 <repository-root> <source-commit> <source-tree> <destination>" >&2
  exit 2
fi
...
python3 -I - "$TMP/archive-attributes" <<'PY'
...
python3 -I - "$TMP/source-tree-entries" <<'PY'
from pathlib import Path
import sys
...
python3 -I - "$TMP/source-tree-entries" "$DESTINATION" "$source_object_format" <<'PY'
from pathlib import Path
import hashlib
import os
import stat
