#!/usr/bin/env python3
"""Exercise the aggregate's bias/derivative prefix with inert call-recording substitutes.

No publication builder, PDF parser, renderer, or prover runs.
All registration guards, the inventory return, and all eight early calls run as one
...
FIELDS = (
    ...
    'GRADIENT_REGISTRATION', 'GRADIENT_REGISTRATION_SHA256', 'GRADIENT_WORK_DIR',
    'CUSP_REGISTRATION', 'CUSP_REGISTRATION_SHA256', 'CUSP_WORK_DIR',
...
def block(source, label):
    start = '# BEGIN DERIVATIVE ' + label + '\n'
    end = '# END DERIVATIVE ' + label + '\n'
...
def main():
    ...
    need(preflight_begin < preflight_end < bias_bindings < inventory < bias_calls
         ...
         'all binding guards must precede both bias and derivative work')
    prefix = source[preflight_begin:finish]
    need(source.count('scripts/build-mgw-derivative-notes-pdf.py') == 3,
         'unexpected derivative production call outside the reviewed blocks')
    need(source.count('scripts/check-mgw-derivative-notes-pdf-self-test.py') == 2,
         'unexpected derivative control call outside the reviewed blocks')
    ...
    passed = []
    with tempfile.TemporaryDirectory(prefix='pid-rs-derivative-dispatch-') as temporary:
        work = Path(temporary)
...
''')
        ...
        environment_values = {'PID_RS_DERIVATIVE_' + key: value
                              for key, value in values.items()}
        environment_values.update({'PID_RS_BIAS_' + key: value
                                   for key, value in bias_values.items()})
        diagnostics = {'PID_RS_DERIVATIVE_' + key:
                       'PID_RS_DERIVATIVE_' + key + ' is required' for key in FIELDS}
        ...
        all_expected = bias_expected + expected
        run('exact eight-call order and bindings', prefix, '--exact', [0] * 8, all_expected, 0)
        ...
        # Causal negative control: reproduce the old placement with inert calls only.
        # The same absent derivative field must expose four consumed bias calls here,
        # while the actual prefix above requires zero calls for that field.
        late = prefix.replace(preflight, '', 1).replace(exact, preflight + '\n' + exact, 1)
        field = 'PID_RS_DERIVATIVE_CUSP_WORK_DIR'
        run('late derivative preflight consumes four bias calls', late, '--exact', [0] * 4,
            bias_expected, 1, omit=(field,), diagnostic=diagnostics[field])

        cross_calls = [['-I', '-S', '-B', 'scripts/build-mgw-derivative-notes-pdf.py',
                        '--kind', kind, '--cross-toolchain'] for kind in ('gradient', 'cusp')]
        ...
        for failing in range(2):
            for code in (0, 1, 3):
                kind = ('gradient', 'cusp')[failing]
                diagnostic = (kind + ' derivative-note cross-toolchain mode unexpectedly accepted'
                              if code == 0 else kind + ' derivative-note refusal returned ' + str(code))
