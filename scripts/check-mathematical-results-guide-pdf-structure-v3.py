...
# This v3 profile binds all 220 navigation records and 53,670 payload bytes of the current
# 24-page guide. The digest is a source-specific artifact census, not a mathematical or
# accessibility claim.
EXPECTED_NAVIGATION_SHA256 = '4d3914cf914496ed2aff921f8676cf39ce2b73161285e2164c10b83d825c1a44'
EXPECTED_STRUCTURE_ELEMENTS = 1069
# This v3 profile binds 2,278 semantic structure records and 116,820 payload bytes. It covers the
# tagged hierarchy, page resources, content, MCIDs, ParentTree ownership, and OBJR topology.
EXPECTED_STRUCTURE_SHA256 = '7231f57e615831ab41d8070c2247b23b8bf2b80d8a4fcbeed737198bc24bd0f8'
...
def validate_named_destinations(
    ...
    def visit(
        ...
        if expected_leaf_count is not None:
            ...
            result = []
            for index, leaf_count in enumerate((32, 32, 3)):
...
def validate_page_tree(reader: PdfReader, root: DictionaryObject) -> None:
    ...
    require_integer(dictionary_raw(pages, "/Count"), 24, "catalog Pages /Count")
    kids = require_array(dictionary_raw(pages, "/Kids"), 3, "catalog Pages /Kids")
    expected_group_counts = (10, 10, 4)
    expected_page = 0
...
def validate_pages_and_links(
    ...
    if len(reader.pages) != 24:
        fail("page_count", f"expected 24 pages, observed {len(reader.pages)}")
    targets: set[str] = set()
    navigation: list[str] = []
...
def validate_parent_tree(
    ...
    *,
    ...
    if set(mappings) != expected_keys or len(mappings) != 122:
