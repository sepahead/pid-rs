...
EXPECTED_OUTLINE = [
    ...
    (0, 15, "6. Estimator choice, global nonclaims, and further reading", "section*.19"),
]
...
DECLARED_ACTIONS = {"/GoTo", "/GoToR", "/URI"}
...
def validate_structure_tree(reader: PdfReader, root: DictionaryObject) -> StructureIndex:
    ...
    if not isinstance(kids, ArrayObject) or len(kids) != 17:
        fail("structure_tree", "IDTree root does not have the declared 17 leaves")
...
def validate_named_destinations(
    ...
    def visit(
        ...
        if expected_leaf_count is not None:
            ...
            if not isinstance(kids, ArrayObject) or len(kids) != 2:
                fail("name_tree", f"{path}: expected exactly two destination leaves")
            result = []
            for index, leaf_count in enumerate((32, 7)):
...
def validate_page_tree(reader: PdfReader, root: DictionaryObject) -> None:
    ...
    require_integer(dictionary_raw(pages, "/Count"), 16, "catalog Pages /Count")
    kids = require_array(dictionary_raw(pages, "/Kids"), 2, "catalog Pages /Kids")
    expected_group_counts = (10, 6)
    expected_page = 0
...
def validate_pages_and_links(
    ...
    if len(reader.pages) != 16:
        fail("page_count", f"expected 16 pages, observed {len(reader.pages)}")
    targets: set[str] = set()
    navigation: list[str] = []
    ...
    for page_number, page in enumerate(reader.pages, 1):
        allowed_page_keys = expected_page_base | ({"/Annots"} if "/Annots" in page else set())
        ...
        navigation.append(
            ...
            f"MediaBox={','.join(geometry_tokens)}"
        )
    ...
    if total_links != 91 or counts != EXPECTED_LINK_COUNTS:
        fail("link_count", f"link action counts changed: {dict(counts)}")
    if len(targets) != 43:
        fail("link_count", f"expected 43 distinct external/file targets, observed {len(targets)}")
...
def validate_parent_tree(
    ...
    if set(mappings) != expected_keys or len(mappings) != 85:
...
def validate_action_owners(
    ...
    if len(owners) != 109:
        fail("action_count", f"expected 109 explicit action owners, observed {len(owners)}")
    edges: dict[
        tuple[tuple[str, int, int] | tuple[str, int], str],
        tuple[tuple[str, int, int] | tuple[str, int], str, str],
    ] = {}
