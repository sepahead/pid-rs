#!/usr/bin/env python3
"""Capture bounded GitHub observations for the composite-v7 lifecycle.

``predecessor_failure`` preserves exact C6 attempt-1 observations and can never
qualify R6. ``successor_qualification`` requires three fresh attempt-1 terminal
success runs for one caller-supplied C7 subject. The implementation reuses only
checksum-bound immutable-v6 transport primitives. Provider bytes do not
...
if not (
    ...
    print(
        "ERROR: capture-ksg-m1a-composite-v7.py requires Python 3.11+ -I -S -B",
        file=sys.stderr,
    )
    raise SystemExit(2)
...
SCRIPT_RELATIVE = "scripts/capture-ksg-m1a-composite-v7.py"
V6_RELATIVE = "scripts/capture-ksg-m1a-composite-v6.py"
V6_PATH = ROOT / V6_RELATIVE
V6_SHA256 = "8089cce79cd9ff14e9eda1b46c51a746b508b026f2cbf5a83cb295e7860d7efa"
V6_SIZE_BYTES = 41_770
...
PREDECESSOR_RUNS = {
    ...
    "predecessor_codeql": 32_139_921_184,
    "predecessor_contract": 32_139_920_743,
}
...
class BootstrapError(RuntimeError):
    """The immutable v6 primitive source could not be loaded exactly."""
...
def read_bound_v6() -> bytes:
    ...
    bootstrap_require(
        ...
        "immutable v6 capture primitive metadata changed",
    )
    ...
    try:
        ...
        bootstrap_require(
            ...
            "opened immutable v6 primitive identity changed",
        )
        chunks: list[bytes] = []
        remaining = opened.st_size
        while remaining:
            chunk = os.read(descriptor, min(remaining, 1024 * 1024))
            bootstrap_require(chunk != b"", "short immutable v6 primitive read")
            chunks.append(chunk)
            remaining -= len(chunk)
        bootstrap_require(os.read(descriptor, 1) == b"", "immutable v6 primitive grew")
    ...
    for field in (
        ...
        bootstrap_require(
            ...
            == getattr(after, field),
            "immutable v6 primitive changed while read",
        )
    raw = b"".join(chunks)
    bootstrap_require(
        hashlib.sha256(raw).hexdigest() == V6_SHA256,
        "immutable v6 capture primitive digest changed",
    )
    return raw


def load_bound_v6(raw: bytes) -> types.ModuleType:
    module = types.ModuleType("pid_rs_capture_ksg_m1a_composite_v6_primitives")
...
try:
    ...
    print("ERROR: unexpected immutable-v6 primitive load failure", file=sys.stderr)
    raise SystemExit(2) from None
...
def expected_successor_artifact_names(role: str, head: str) -> set[str]:
    ...
    return {f"ksg-m1a-composite-v7-static-{head}"}
...
# mutate the immutable v6 primitive bytes or its other bounds/transport rules.
V6.workflow_identity = workflow_identity
V6.expected_successor_artifact_names = expected_successor_artifact_names
...
def capture_configuration(
    ...
    if arguments.phase == "predecessor_failure":
        ...
        require(
            ...
            "terminal C6 predecessor run roster is unresolved",
        )
        return (
            dict(PREDECESSOR_RUNS),
            dict(PREDECESSOR_CONCLUSIONS),
            {role: C6_COMMIT for role in PREDECESSOR_RUNS},
            {"predecessor_commit": C6_COMMIT, "predecessor_tree": C6_TREE},
        )
    ...
    return (
        ...
        {
            "predecessor_commit": C6_COMMIT,
            "predecessor_tree": C6_TREE,
...
def offline_self_test() -> dict[str, Any]:
    ...
    require(primitive.get("result") == "pass", "immutable v6 primitive self-test failed")
    ...
    for role, (name, path, event, conclusion) in exact_routes.items():
        ...
        require(
            V6.validate_run_document(fixture, role, run_id, head, conclusion) == 17,
    ...
    for role, fixture, stale_name, stale_path, conclusion in (
        ...
        try:
            V6.validate_run_document(stale, role, run_id, head, conclusion)
        except CaptureError:
    ...
    exact_artifact_sets = {
        ...
        "successor_contract": {f"ksg-m1a-composite-v7-static-{head}"},
    }
    require(
    ...
    stale_artifact = {
        "expired": False,
        "id": 201,
        "name": f"ksg-m1a-composite-v6-static-{head}",
        "size_in_bytes": 1,
        "workflow_run": {
            "head_branch": "main",
    ...
    for role in exact_artifact_sets:
        ...
        hostile["name"] = (
            f"ksg-m1a-composite-v6-static-{head}"
            if role == "successor_contract"
            else "wrong-artifact"
        )
        try:
            V6.capture_artifacts(
                role,
                [hostile],
                run_id,
                17,
    ...
    return {
        ...
        "immutable_v6_capture_self_test": "pass",
        "phase_aware_workflow_routes_verified": 6,
        "predecessor_terminal_roster_bound": PREDECESSOR_RUNS
        == {
            "predecessor_ci": 32_139_920_717,
            "predecessor_codeql": 32_139_921_184,
            "predecessor_contract": 32_139_920_743,
        },
        "result": "pass",
        "schema": "pid-rs/ksg-rev4-m1a-composite-v7-capture-self-test/v1",
...
def main() -> int:
    ...
    try:
        ...
        current_v6 = V6.read_regular(V6_PATH, V6_SIZE_BYTES, 0o644)
        require(current_v6 == V6_RAW and sha256(current_v6) == V6_SHA256, "immutable v6 primitives changed")
        ...
        token = V6.read_token(arguments.token_fd)
        script_raw = V6.read_regular(SCRIPT, 1024 * 1024, 0o644)
        ...
        for repetition in (1, 2):
            for role in sorted(runs):
                artifacts, failed_job_ids, repository_id = V6.capture_run(
                    role,
                    runs[role],
                    heads[role],
                    conclusions[role],
        ...
        require(len(captures) <= V6.MAX_CAPTURE_ROWS, "capture response count exceeds bound")
        require(
            sum(row["body_size_bytes"] for row in captures) <= V6.MAX_CAPTURE_BODY_BYTES,
            "retained provider bodies exceed bound",
        )
        rendered = canonical_json(result)
        require(len(rendered) <= V6.MAX_CAPTURE_BYTES, "capture document exceeds bound")
