...
HEADER="$ROOT/audit/formal/latex/cross-implementation-reverification/header.tex"
...
validate_pdf() {
  ...
  python3 -I -B - "$candidate" "$expected_trailer_id" "$EXPECTED_PAGES" <<'PY'
import sys
...
expected_action_counts = {"/GoTo": 41, "/GoToR": 0, "/URI": 37}
