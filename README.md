...
| Assess implementation and compute cost | [Rust implementation and cost](RUST_IMPLEMENTATION_AND_COST.md) | Current APIs, source-derived time and memory costs, resource limits, and unimplemented research routes |
| Study the mathematics | [Mathematical results guide](MATHEMATICAL_RESULTS_GUIDE.md) | Nine main result families and six companion packages, with motivation, assumptions, evidence and complete proof links |
...
| Re-check the estimators | [Cross-implementation re-verification](audit/evidence/cross-implementation-reverification-2026-09-26.md), [PDF](output/pdf/cross-implementation-reverification.pdf) | A second from-definition implementation of the categorical and continuous estimators, an exact test of a repository bound, one fixed percentile defect and one citation correction; model review, not independent review |
...
| Tolerate faulty sensors | [Fault-tolerance note](audit/research/fault-tolerant-shared-exclusions/EXPOSITION.md), [PDF](output/pdf/fault-tolerant-shared-exclusions.pdf) | MGW events as fault-consistency sets, a Lean-checked validity bound, exact non-monotone examples and a fault-injection test with negative worst-case and wrong-budget results; development evidence only |
