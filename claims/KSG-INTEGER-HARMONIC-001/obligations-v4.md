...
typed digamma premise + signs + exclusive/inclusive index maps
  ...
  -> E exact identity and sharp rectangular-arithmetic-domain signed local bound
...
Lean(19 theorems, 14 mutants)
  ...
  + SMT checker firewall(52 separate controls)
  + explicit shared-cut accounting
  -> F scoped formal assurance

claim custody + phase isolation + source/compiled replay
  + catalog/release/audience/identity preclosure work
  + M1a-valid settled CI and hostile review
  -> preclosure implementation candidate

preclosure implementation candidate
  + unsigned canonical M1a implementation commit
  + push, remote verification, and receipt
...
| E3 | Prove harmonic monotonicity and the sharp rectangular-arithmetic-domain bound `-D<=T<=D`. | GO for exact object | Lean rational theorem and real bridge; signed values allowed; no runtime-image equality |
...
| E5 | Qualify `x+y<=n+k` and its balanced lower bound. | conditional source lemma; not promoted | set derivation assumes finite positive max-product radius, unique shell, exact strict counts, and correct mapping; formal/refinement/compiled/mutation/provenance lanes remain open |
...
| N1 | Separate the 8-epsilon rounded-reference comparator from exact-rational error. | correction active; final replay open | exact-rational maximum enclosed below 9.761311 eps; both finite-corpus only |
...
| M2 | Retain rejected-prime collision as a negative control. | GO | four exact-nonzero collisions at prime `1000003`; the odd-prime identity `H_(p-1-t)=H_t mod p` explains the shared event but not selected-field separation |
| M3 | Kill 28 modular certificate mutations. | GO in normal and optimized Python | 2/2 strict-JSON controls pass separately; bounded certificate/custody adequacy only |
