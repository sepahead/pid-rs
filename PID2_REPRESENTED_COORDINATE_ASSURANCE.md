...
The new revision-4 evidence is deliberately narrower than an estimator theorem. An independent
integer-and-`Fraction` checker derives binary64 expectations without performing host floating-point
...
separate scopes. A hostile self-test forbids host-float oracle shortcuts, runs from a copied root,
and requires independent semantic and custody mutations to fail closed.
...
- the independent executable checker, hostile mutations, custody scopes, and API parity checks.
...
$$
U_{1,64}=\operatorname{RN}_{\rm even}(I_{1,64}-R_{64}),\qquad
U_{2,64}=\operatorname{RN}_{\rm even}(I_{2,64}-R_{64}),
$$

and

$$
S_{64}=\operatorname{RN}_{\rm even}
\left(\operatorname{exact}(J_{64}-I_{1,64}-I_{2,64}+R_{64})\right).
$$
...
$$
d(\operatorname{RN}_{\rm even}(R_{64}+U_{1,64}),I_{1,64})\le 32,
$$

$$
d(\operatorname{RN}_{\rm even}(R_{64}+U_{2,64}),I_{2,64})\le 32,
$$

and

$$
d(\operatorname{RN}_{\rm even}(R_{64}+U_{1,64}+U_{2,64}+S_{64}),J_{64})\le 32.
$$
...
| Python integer/`Fraction` model | Independent language and representation | Selected oracle; still shares the human IEEE specification. |
...
- requires 18 independent semantic mutations to fail through the checker's declared error path,
...
Rust extension. The independent expected route must reconstruct `U1`, `U2`, and `S` from the report's
...
algebra, and the declared Möbius rows. The hostile self-test must independently mutate:
...
## 14. Thirty-lens adversarial review
...
| 17 | Python API | Scalar/report parity uses independent integer-ratio expectations. |
| 18 | Debug/release | Focused compiled partitions run in both profiles. |
| 19 | Oracle independence | Python integer grid does not call the production reducer. |
...
Run the independent model and hostile self-test under both interpreter modes:
...
Python binding parity tests, the independent Z3 mutations, method-catalog coherence, formatting,
lint, broader workspace tests, source-state regeneration, a clean committed tree, and direct remote
OID verification.

No publication PDF is created in this assurance milestone. A later publication milestone may
render this document only after Markdown/PDF semantic parity and visual inspection; absence of a
PDF here prevents a partially synchronized publication artifact.
