...
The two reports connect categorical Makkeh–Gutknecht–Wibral shared exclusions, sensor availability and predictive utility. They state the assumptions, give complete finite examples, and explain the current Rust implementation and computational cost. Four classical finite-PMF foundations and a separate 13-target conditional continuation have local Lean checks and fresh-kernel replays; their [exact scope and source snapshots](../../formal/lean-finite-logscore/PUBLICATION.md) remain separate from the handwritten availability/mask and MGW application argument. These checks do not establish portable or hosted replay, Rust refinement, estimator calibration or sensor benefit. The reports contain no new timing measurement, implemented learning loop or sensor benchmark. The availability objective is determined by subset mutual information; a full PID needs a separately tested explanatory benefit.
