...
INPUT_PINS_SHA = 'e257f1b32dfb6cb04ae1dac20db65142c22a64b050292da67a552560dacdb95c'
MODULES = (
    'PidFiniteConvergence/Deterministic', 'PidFiniteConvergence/SxEventBridge',
    ...
    'MgwBridgeDepsV1/JoinLog/Proofs', 'PidMgwBridge/Contract', 'PidMgwBridge/RawTargets',
    'PidMgwBridge/AliasTargets', 'PidMgwBridge/JudgeCore', 'PidMgwBridge/Candidate',
    'PidMgwBridge/SemanticJudge', 'PidPrefixProbability/Contract',
    'PidPrefixProbability/RawTargets', 'PidPrefixProbability/AliasTargets',
    'PidPrefixProbability/Candidate', 'PidPrefixProbability/SemanticJudge',
    'PidPrefixMgwMean/Contract', 'PidPrefixMgwMean/RawTargets',
    'PidPrefixMgwMean/AliasTargets', 'PidPrefixMgwMean/Candidate',
...
CASES = [
    ...
    ('dispatch-six-as-mean', 'failed', 'record count'),
    ('dispatch-eleven-as-prefix', 'failed', 'record count'),
    ('dispatch-three-as-mgw', 'failed', 'record count'),
    ('mean-reordered', 'failed', 'identity differs'),
    ('mean-type-drift', 'failed', 'exact accepted mean records differ'),
    ...
    ('wrong-final-missing-prefix', 'failed', 'retain the first two exact'),
...
def fixture(directory, name):
    ...
    stage = root / '.local/prefix-mgw-mean-public-replay-v1/control-stage'
    ...
    controls = {'schema': 'pid-rs/portable-mean-controls-acceptance-v1',
                'status': 'adopted_completed_portable_mean_controls',
    ...
    registration = {
        'schema': 'pid-rs/prefix-mgw-mean-packaging-registration-v1',
    ...
    elif name == 'manifest-digest-type': manifest['files']['README.md'] = False; changed_manifest = True
    elif name == 'manifest-missing-member':
        del manifest['files']['sources/PidPrefixMgwMean/RawTargets.lean']; changed_manifest = True
    elif name == 'selected-hardcoded-pin':
        path = package / 'sources/PidPrefixMgwMean/Candidate.lean'
        path.write_bytes(path.read_bytes() + b'\n-- synthetic pin drift\n')
        manifest['files']['sources/PidPrefixMgwMean/Candidate.lean'] = digest(path.read_bytes())
        changed_manifest = True
    elif name == 'package-member-drift':
        with (package / 'README.md').open('ab') as stream: stream.write(b'\nsynthetic drift\n')
    elif name in {'package-member-symlink', 'package-member-hardlink'}:
        path = package / 'README.md'; target = directory / 'member-target'; path.rename(target)
    ...
    if name.startswith('adoption-count-'):
        controls['new_mean_theorems_proved'] = {'bool': False, 'float': 0.0, 'one': 1}[name.rsplit('-', 1)[1]]
    ...
    if name in {'ledger-child-cap', 'child-cap-between-children'}:
        ledger['children'] = [{'kind': 'compiler_kernel'} for _ in range(150 if name == 'ledger-child-cap' else 149)]
...
def main():
    ...
    require(registration['schema'] == 'portable-mean-adapter-controls-execution-v1' and
            ...
            and registration['new_mean_theorems_proved'] == 0,
    ...
    result = {'status': 'failed', 'scope': 'synthetic whole-entrypoint controls; no proof or adoption',
              ...
              'new_mean_theorems_proved': 0, 'records': []}
    try:
        for mode in (0, 1):
            for name, expected_status, diagnostic in CASES:
                ...
                if expected_status == 'fresh_semantic_kernel_pass':
                    ...
                    labels += ['compile-PrefixMgwMeanCandidateRoot', 'fresh-kernel']
                    require([x['label'] for x in calls] == labels, name + ': exact 26-dispatch order differs')
                    require(len(body['semantic_records']) == 3 and len(body['accepted_prefix_dependency_records']) == 6
                            and len(body['accepted_mgw_dependency_records']) == 11, 'rank record dispatch differs')
                    require(calls[-1]['command'][1:] == ['--fresh', 'PrefixMgwMeanCandidateRoot'], 'fresh kernel argv differs')
                ...
                if expected_status == 'expected_target_rejection':
                    require(len(calls) == 24 and calls[-1]['label'] == 'compile-PidPrefixMgwMean-SemanticJudge',
                            'wrong target dispatch must stop at child 23 plus one probe')
        ...
        for mode in (0, 1):
            ...
            result['records'].append({'name': 'shared-stage-mode', 'mode': mode,
                                      'status': 'matched_expected_behavior', 'synthetic_runner_calls': 26})
        ...
        require([x['python_optimized'] for x in shared_ledger['attempts']] == [0, 1]
                and len(shared_ledger['children']) == 52, 'shared normal/O ledger routing differs')
        for rel, expected in pins.items():
