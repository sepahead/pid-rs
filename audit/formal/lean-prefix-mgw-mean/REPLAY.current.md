...
For the publication builder, provide Pandoc 3.10.2, the reviewed LuaLaTeX/TeX Live environment and exact publication assets, then register a fresh externally supervised build stage. The builder retains its work directory and all command streams. Each command has a 300-second bound; an external stage owner must provide aggregate limits and cleanup review. Its current profile binds the corrected nine-page discovery reference `93ff18e21f56e4770b12b7405d57b003bf7839cef5c4207ed98a56cd6f96f58f`. Root has separately admitted that artifact's correspondence. The [current local reproduction record](CURRENT_REPRODUCTION.md) documents both successor control modes and four exact builds of those bytes on 12 September 2026. The older normal and optimized checks belong to their older PDF hash. Exact mode admits only the current pinned bytes, with no source-to-installed-font authenticity claim.
...
These are command shapes for a new run; the current local record above contains the completed current-hash runs. `--check` makes two new build directories. Optional `--output` must be a new file; an interrupted output write can leave a partial owned output for inspection. Existing paths are not overwritten. `--cross-toolchain` deliberately returns status 2 before building because no reviewed relation for this mean PDF exists. A successful same-toolchain run establishes only the exact publication relation it checks. The [v2 control distribution](pdf-controls/CONTROLS.md) preserves its 224 actual cases and separate relocation check with the same 224-case/532-start scope. The [v3 successor source](pdf-controls/CONTROLS.v3.md) adds two companion-link negatives and binds the corrected corpus. Both actual outer modes later passed that declared 232-case/548-start matrix on 12 September 2026; the source archive alone supplies no execution credit. Full integration inventories, hosted evidence and source-state generation remain separate obligations.
