...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\footnotesize\color{PidInk}\bfseries Represented-binary64 assurance}
\fancyhead[R]{\footnotesize\color{PidTeal}exact reduction · quantizer reachability · hard boundaries}
\fancyfoot[L]{\footnotesize\color{PidSlate}Sepehr Mahmoudian · 31 August 2026}
...
\fancypagestyle{plain}{%
  \fancyhf{}
  \fancyhead[L]{\footnotesize\color{PidInk}\bfseries Represented-binary64 assurance}
  \fancyhead[R]{\footnotesize\color{PidTeal}mathematical and implementation assurance}
  \fancyfoot[L]{\footnotesize\color{PidSlate}Sepehr Mahmoudian · 31 August 2026}
  \fancyfoot[R]{\footnotesize\color{PidInk}\thepage}
  \renewcommand{\headrulewidth}{0.25pt}
}
  ...
  \begin{titlepage}
    ...
    \begin{minipage}{0.91\textwidth}
      ...
      {\fontsize{33}{39}\selectfont\bfseries\color{PidIvory}%
        \mbox{Represented-binary64}\par
        \mbox{assurance for PID}\par}
    \end{minipage}\par
    \vspace{13mm}
    {\Large\bfseries\color{PidMineral}Exact reduction, quantizer reachability, and evidence limits\par}
    \vspace{43mm}
    {\small\bfseries\color{PidTeal}\MakeUppercase{represented inputs · one rounding · explicit nonclaims}\par}
    \vspace{8mm}
    \begin{minipage}{0.86\textwidth}
      \large
      A self-contained account of the 34-limb exact represented-input reducer, the witnessed PID
      call-site repairs, finite-binary64 quantizer reachability, exact examples, costs, oracle
      structure, rejected transfers, application scenarios, and remaining proof obligations.
    \end{minipage}\par
    \vfill
    \begin{minipage}{0.88\textwidth}
      \small\color{PidSlate}
      The Markdown and two SVG files are canonical. The PDF is a human-readable projection.
      Exact addition of represented operands is not exact estimation or application validity.
    \end{minipage}\par
    \vspace{12mm}
    ...
    \vspace{3mm}
    {\small\color{PidSlate}Canonical source: \texttt{NUMERICAL\_ASSURANCE.md}\par}
    \vspace{17mm}
  \end{titlepage}
