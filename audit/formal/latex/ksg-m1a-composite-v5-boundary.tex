...
repository CI finished with 41 successful and four failed jobs, and the dedicated v4 route failed
at the static-validation preflight before substantive contract validation completed. This report
records why R4 is permanently unissued, defines an append-only C5
successor with five bounded repairs and a fresh Lean replay, and permits R5 only after fresh
...
\midrule
Repository CI & \texttt{32079866560}; failed & Forty-one successful and four failed jobs; the all-jobs-success term is false. \\
...
Legacy recovery wiring & Exact stale README-token expectation & Replace only the stale receipt-child
README tokens with current nonissuance and v5/R5 terms; retain Git, custody, workflow, Just, and
anti-revival checks. \\
\bottomrule
\end{tabularx}
...
The zeta change is spelling custody only. Neither the certified-SxPID nor recovery-wiring rebind
expands mathematical scope, issues or renames a receipt, changes a prior hosted outcome, or
transfers scientific credit.
...
\includegraphics[
  width=\linewidth,
  alt={C4 is published but its CI and dedicated v4 routes failed, so Q4 is false and R4 is
  unissued. A direct-child C5 makes five bounded repairs, including replacement of a stale live
  README-token expectation without retiring the legacy checker's historical Git or custody logic.
  Only fresh attempt-1 local, CI, CodeQL, and dedicated-v5 success can make Q5 true and permit R5,
  whose evidence remains process custody rather than scientific authority.}
