...
bound and its source, target-restricted, and target-event corollaries.

Lean does not yet encode empirical probability laws, stochastic limit theorems, the
redundancy-lattice antichain order and concrete M{\"o}bius inverse, complete SxPID components,
cumulatives, atoms and averaging, $\imin$, Rust refinement, or binary64 arithmetic.  The checker
...
kernel checker.  It enforces an exact ordered inventory of all 225 source-level declarations
...
positivity, fractional-cover, and generic M{\"o}bius claims.  CI runs the same checks.
