This note explains the finite-row expectation bridge for the categorical shared-exclusions PID of Makkeh, Gutknecht and Wibral (MGW). The three exact targets were accepted locally on 8 September 2026 after the complete semantic and fresh-kernel matrix. The portable replay was also accepted locally after four complete proof replays and the required rejection of an altered final theorem. Hosted replay and mainline publication remain separate. The [verification record](https://github.com/sepahead/pid-rs/blob/main/audit/evidence/prefix-mgw-mean-formal-verification-2026-09-08/RESULTS.md) separates those statuses.

The practical question is precise: can a statistic that observes complete categorical rows, rather than a supplied probability table, have an expectation that approaches a specified MGW atom? The three accepted theorems answer that question for a particular prefix statistic. These three theorems alone do not give a finite-horizon planning bound. A short prefix can have the wrong sign even when the true atom is zero or negative.
...
For an event $A\subseteq\mathcal K$, $P(A)=\sum_{x\in A}p(x)$. Write $x_i$ for source $i$'s coordinate and $t_x$ for the target coordinate. The target random variable is $T(x)=t_x$ under this law.
...
The full finite node set is used in this sum. Restricting it to a few selected nodes generally changes the inverse problem. To recover $f$ from a prescribed cumulative $g=\mathcal Zf$, process lower nodes first and use

$$
f(\beta)=g(\beta)-\sum_{\alpha\prec\beta}f(\alpha),
$$

where $\alpha\prec\beta$ means $\alpha\preceq\beta$ and $\alpha\ne\beta$. This finite recursion gives both existence and uniqueness; it is the lower-cumulative, or Möbius, inversion used below.
...
Set this event to false for an empty list or any list containing an empty mismatch set. Thus a complete source match makes the event false without requiring a join of invalid nodes. The indicator $\mathbf1\{F\}$ equals one when $F$ holds and zero otherwise.
...
![A three-draw block showing raw time, target filtering and retained rank](retained-rank.pdf)
...
To prove the identity, first sum the coefficient over all nodes below $\beta$. For one row with nonempty $M_z(x)$, its singleton family lies below $\beta$ exactly when every collection $C\in\beta$ contains an index in $M_z(x)$. This says that every collection has a mismatch, which is precisely $x\notin E_\beta(z)$. A join lies below $\beta$ exactly when every joined factor does. If a row has empty mismatch set, it belongs to $E_\beta(z)$ and contributes to neither side. Hence a valid repeated join lies below $\beta$ exactly when each row is outside $E_\beta(z)$. Finite distributivity therefore gives
...
With weights $r=p$, finite expectation gives $u_j^z(\alpha)=c_j^z(\alpha)/j$: multiply each increment by its word mass and sum over the words. At a supported anchor, the coefficient identity therefore gives
...
There are $\binom{j-1}{k}$ choices of the previous matching positions. For any such pattern with positive probability, the retained rows are independent draws from the row law conditioned on $T=t_z$; zero-probability patterns contribute zero. Thus $1-q_\beta/v$ is their source-event exclusion probability. The retained-rank denominator stays inside the sum.
...
For a sensor study, this is a foundation for an estimator design, not a recommendation to deploy the prefix statistic. Before use, the implementation still needs a refinement argument, numerical and resource contracts, a sampling-error bound and tests against appropriate alternatives. Direct held-out task loss, MI/CMI, fixed-model and retrained ablation, failure utility and coverage can answer useful sensor questions with fewer assumptions. MGW is relevant when its particular redundant/unique/synergistic allocation answers a question those summaries leave open.

The locally accepted [finite-horizon bias companion](https://github.com/sepahead/pid-rs/blob/main/audit/formal/lean-prefix-mgw-bias/EXPOSITION.md) bounds the omitted positive and negative tails through the overlap probability $q_\alpha(z)$. It gives supported-population moment, projection-support and mass-floor bounds under their stated assumptions. These control mean truncation error; they do not establish sampling-error calibration or refinement to the Rust implementation. A Boolean truth-table representation of node joins remains a separate computational refinement obligation.
