...
\PidMeanSection{1. Data, law and information question}\label{data-law-and-information-question}
...
\PidMeanSection{2. A node is a collection of alternatives}\label{a-node-is-a-collection-of-alternatives}
...
\PidMeanSection{3. What the prefix statistic observes}\label{what-the-prefix-statistic-observes}
...
\PidMeanSection{4. The finite coefficient identity}\label{the-finite-coefficient-identity}
...
\PidMeanSection{5. Expectations under the actual row law}\label{expectations-under-the-actual-row-law}
...
\PidMeanSection{6. Why the mean converges to the named atom}\label{why-the-mean-converges-to-the-named-atom}
...
The formal proof uses the pinned Mathlib theorem \texttt{Real.hasSum\_pow\_div\_log\_of\_abs\_lt\_one} with \(|1-x|<1\). The endpoint \(x=1\) gives a zero series. This is a classical logarithm identity, not a project novelty.
...
\PidMeanSection{7. A sensor with no target information can give a positive short-prefix mean}\label{a-sensor-with-no-target-information-can-give-a-positive-short-prefix-mean}
...
\PidMeanSection{8. Edge cases and permitted interpretations}\label{edge-cases-and-permitted-interpretations}
...
\PidMeanSection{9. What this adds, and what must come next}\label{what-this-adds-and-what-must-come-next}
...
The locally accepted \href{https://github.com/sepahead/pid-rs/blob/main/audit/formal/lean-prefix-mgw-bias/EXPOSITION.md}{finite-horizon bias companion} bounds the omitted positive and negative tails through the overlap probability \(q_\alpha(z)\). It gives supported-population moment, projection-support and mass-floor bounds under their stated assumptions. These control mean truncation error; they do not establish sampling-error calibration or refinement to the Rust implementation. A Boolean truth-table representation of node joins remains a separate computational refinement obligation.
