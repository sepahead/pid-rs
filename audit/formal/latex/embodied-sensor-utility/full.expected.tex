...
The true law is \(r\); the forecast is \(q\). Require \(q_y>0\) for every \(y\in S\). Outside \(S\), \(q_y\) may be zero or positive. Neither full support nor a sampling or independence assumption is required. The \href{https://github.com/sepahead/pid-rs/blob/main/audit/formal/lean-finite-logscore/PUBLICATION.md}{finite log-score formal package} records the four exact targets, proof sources and local replay scope. They establish this unconditional finite-law foundation, not a conditional sensor theorem or a new PID result.
