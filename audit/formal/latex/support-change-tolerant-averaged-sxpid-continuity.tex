...
separately, all 24 supplied exact-count two-source informative, misinformative, and signed-net
cumulative and concrete M{\"o}bius-atom coordinates with exact products and sign and zero
equivalences.  It does not check the support-change logarithmic transfer between laws, the
published component-atom nonnegativity theorem, bytes-to-counts, Rust or floating-point
...
The exact functions \(g_J,e_K^\vee,e_K^\Sigma\) are not all monotone on \([0,1]\).
...
For \(K\ge2\), to use only an upper radius \(0\le\eta\le\varepsilon\le1\), define
\begin{equation}
...
nodes.  A second checked bridge fixes the complete two-source 24-coordinate cumulative/atom
surface, proves its M{\"o}bius/zeta and averaging algebra, and derives exact rational and real
products with scaled-log sign and zero equivalences.

Lean does not yet bundle two-law semantic total variation, derive the support-change \(g_J\)
transfer from \(T_N\), import the published component-atom nonnegativity theorem, or extend the
bridge to a higher-source lattice.  Bytes or rows to counts, sampling and population claims, Rust
or floating-point refinement, and certifier/parser execution also remain outside the checked
theorem.
