...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{pid-rs averaged categorical SxPID}
\fancyhead[R]{support-change analysis}
\fancyfoot[C]{\thepage}
...
\title{\textbf{Support-Change-Tolerant Continuity\\
for Averaged Categorical Shared-Exclusions PID}\\
\large Closed-simplex total-variation bounds, exact counterexamples,\\
...
Lean checks the exact heterogeneous keyed Sx events and the finite equivalence-union load bound,
but not the complete logarithmic and averaged theorem.  The result does not prove binary64
enclosure, estimator calibration, or continuous PID validity.
\end{abstract}
...
Lean does not yet bundle probability laws or semantic total variation, define the
support-restricted logarithmic functional, derive \(g_J\) from \(T_N\), identify the complete
redundancy lattice and its concrete M{\"o}bius inverse, import the published pointwise sign theorem,
or compose these facts into averaged Sx cumulatives and atoms.  Sampling, Rust refinement, and
binary64 arithmetic also remain outside the checked theorem.
