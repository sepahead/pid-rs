...
% Keep two reviewed section boundaries deterministic.  TeX Live 2024 and the Ubuntu runner can
% otherwise split the numbered mathematical argument one body line apart and resolve the
% actor-attribution lead-in's stretchable space 33.098pt apart.  The exact-title guards scope the
% forced boundaries to those two report paragraphs; the canonical Markdown and every other
% level-four heading are unchanged.
\def\PidWorkflowPortableParagraphTitleOne{Discovery, verification, and communication methods}
\def\PidWorkflowPortableParagraphTitleTwo{Mathematical method reconstructed from the paper}
...
\newcommand{\PidWorkflowReportParagraph}[1]{%
  \begingroup
  \def\PidWorkflowCurrentParagraphTitle{#1}%
...
\markdownSetup{
  ...
  rendererPrototypes = {
    ...
    headingFour = {\PidWorkflowReportParagraph{#1}},
    codeSpan = {\texttt{\seqsplit{#1}}},
    link = {\href{\detokenize{#3}}{#1}},
    inputVerbatim = {
...
then defines the complete project protocol for exact claims, separately developed routes with
explicit independence accounting, retained failures, certificate conversion, adversarial audit,
blind holdouts, PID-specific exceptional
cases, Codex goal and continuity control, a future typed publication harness, layered assurance,
branch/worktree lifecycle, durable storage, and claim disposition. The protocol is project-defined
guidance. It is not a PID estimator,
...
\[
  ...
  =\operatorname{Program}_{\mathrm{compiler,features,platform}}
    (Z_1,\ldots,Z_n),
\]
and a consumer may use that number, a typed diagnostic record \(U_n\), and a policy \(\mathcal D\)
to make a decision
\[
  d_n=\mathcal D(\widetilde F_n,U_n).
\]
Here \(\mathcal D\) has a declared domain consisting of the represented-result and
diagnostic-record types and maps them into a declared decision space containing \(d_n\). These are
four different objects: the population functional, the estimator, the executable
...
\[
  ...
  \tag{1}
\]
where \(\mathcal X\) is the domain, \(k\geq 1\), \(A_i\) are assumptions, and \(C\) is the conclusion.  A
...
\[
  ...
  \exists N(P,\varepsilon,\eta)\in\mathbb N\;
...
Start with two routes to one claim. Route A needs premises A1, A2, and C. Route B needs
premises B1 and C. A route needs all its premises: this is AND. Either fully justified route
can support the claim: this is OR. Each route also needs evidence that its premises imply
the claim. The diagram assumes that these route obligations are accepted.

Both routes depend on C. If C lacks accepted evidence, neither route is accepted. A shared
cut is a set of allowed dependency nodes that meets every route. A cut is inclusion-minimal
if no proper subset still meets every route. Here the three minimal cuts are
\(\{C\}\), \(\{A1,B1\}\), and \(\{A2,B1\}\). The figure fixes which nodes are allowed in a cut.
The GO label states the illustrated gate state; it does not state a new mathematical result.
...
\PidWorkflowSubsubsection{Advanced definition}
...
\PidWorkflowSubsection{Certificates and separately specified checking}
...
related to atoms \(\pi_\beta\) by a finite zeta transform. The orientation below is part of the
definition: \(\beta\preceq\alpha\) means that atom \(\pi_\beta\) contributes to cumulative
\(c_\alpha\).
\[
...
redundancy, source-specific unique terms, and synergy.  Let \(c_R\) be the redundancy-node
cumulative, \(c_1\) and \(c_2\) the two single-source cumulatives, and \(c_{12}\) the joint-source
cumulative for the already declared functional. Inversion is
\begin{align*}
  R   &=c_R,\\
  U_1 &=c_1-R,\\
  U_2 &=c_2-R,\\
...
\[
  ...
  \bigr)
\]
lexicographically, with larger tuples preferred. Thus an accepted invalid mutant outranks every
rejected mutant, and the negative signs prefer smaller witnesses and shorter runtimes within the
same acceptance class. Mutation operators may swap a source mask, replace a union with an
...
\[
  ...
  \tag{7}
\]
The component \emph{assurance-component closure state} is one of
\texttt{not-applicable}, \texttt{open}, \texttt{rejected}, or \texttt{closed}.  It is distinct
from the canonical artifact-verification label, from the accepted evidence class (for example,
theorem, finite machine check, certified bound, empirical result, counterexample, or diagnostic),
and from the claim disposition
...
Let test-result random variables \(T_1,\ldots,T_k\) bear on a binary proposition \(H\), let
\(t_1,\ldots,t_k\) denote one possible result tuple, and let \(C\) range over a fully defined finite
set of latent shared-failure states for an oracle, imported theorem, generator, or implementation.
Different test names do not make them independent.  For each \(B\in\{H,\neg H\}\), a complete joint
evidence model must retain the shared state:
\[
  \Pr(T_1=t_1,\ldots,T_k=t_k\mid B)
  =
  \sum_c
  \Pr(C=c\mid B)\,
  \Pr(T_1=t_1,\ldots,T_k=t_k\mid B,C=c).
  \tag{8}
\]
...
\[
  \Pr(T_1=\cdots=T_5=\mathrm{pass}\mid D\wedge O)=1.
  \tag{9}
\]
