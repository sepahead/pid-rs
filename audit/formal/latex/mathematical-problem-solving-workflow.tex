...
\titleformat{\paragraph}[block]
  {\normalsize\bfseries\color{WorkflowDark}}
  {\theparagraph}{0.55em}{}
...
\def\markdownLaTeXReadAlignments#1{%
  ...
  \ifnum\markdownLaTeXColumnTotal=2\relax
    \addto@hook\markdownLaTeXTableAlignment{%
      >{\raggedright\arraybackslash\small}p{0.455\linewidth}}%
  \else
    \ifnum\markdownLaTeXColumnTotal=4\relax
      \addto@hook\markdownLaTeXTableAlignment{%
        >{\raggedright\arraybackslash\small}p{0.215\linewidth}}%
    \else
      \addto@hook\markdownLaTeXTableAlignment{%
        >{\raggedright\arraybackslash\small}p{0.295\linewidth}}%
    \fi
  \fi
...
\markdownSetup{
  ...
  tableCaptions = false,
  rendererPrototypes = {
    headingOne = {\Needspace{0.26\textheight}\section{#1}\label{sec:canonical-protocol}},
    headingTwo = {\Needspace{0.26\textheight}\subsection{#1}},
    headingThree = {\Needspace{0.26\textheight}\subsubsection{#1}},
    headingFour = {\Needspace{0.26\textheight}\paragraph{#1}},
...
\begin{abstract}
This document is the human-readable typeset rendition of
\texttt{\seqsplit{MATHEMATICAL\_PROBLEM\_SOLVING\_WORKFLOW.md}}. It records external process observations,
...
cases, layered assurance, and claim disposition. The protocol is project-defined guidance. It is
not a PID estimator, theorem, benchmark result, or validation claim.
\end{abstract}
...
\section{Reader's primer: objects, logic, and evidence boundaries}
...
The assurance path therefore has distinct implications:
\[
...
\subsection{Exact claims, quantifiers, and counterexamples}
...
edge's implication has itself been justified.  The packet retains every minimal shared cut set:
each smallest set of open or common-cause nodes that intersects every currently accepted route to
the target.  Rephrasing an open node, or closing one branch of an AND-edge, is not closure.

\subsection{Certificates and independently checkable computation}
...
\[
  ...
  \text{certificate}
  \xrightarrow{\text{independent checker}}
...
\subsection{PID vocabulary and a finite worked reconstruction}

PID studies how several sources \(S_1,\ldots,S_m\) provide information about a target \(T\).
For finite variables, a joint law \(P\) assigns nonnegative masses summing to one.  A source
collection is a nonempty subset of \(\{1,\ldots,m\}\).  An antichain is a nonempty set of source
collections in which no member contains another.  PID cumulative quantities \(c_\alpha\), indexed
by antichains, are related to atoms \(\pi_\beta\) by a finite zeta transform
\[
...
reconstruction obligation only.  Negative signed shared-exclusions atoms are permitted by the
functional and must not be clamped merely to make a table look nonnegative.

\subsection{Negative example: an unseen rare cell}

Let a finite population contain a cell \(z_\star\) with mass
\(P(z_\star)=\rho>0\).  Under independent sampling, the exact probability that \(z_\star\) is
...
\subsection{Negative example: pairwise is not mutual independence}
...
\subsection{Bounded evolutionary search: useful scope and hard boundary}
...
\section{Typed assurance and rigorous evidence aggregation}

\subsection{Status of this supplement}
...
\subsection{Preserve a vector across assurance layers}
...
\subsection{Critical cut sets determine dependence}
...
critical-cut-set memo names each shared node, which routes use it, and which independent
...
\subsection{Worked example: five correlated test families are one confirmation at a shared oracle}
...
mean that the shared oracle independently contains the same defect.  Conditional on
...
\(E_{\mathrm{num}}\) & Oracle values reproduce their own fixtures & No independent enclosure yet \\
...
\subsection{When a model-conditional posterior belief is permitted}
...
\begin{enumerate}
  ...
  \item finite-calibration uncertainty and model-shift uncertainty inside a sensitivity region
  \(\Theta\), together with uncertain priors, likelihoods, and dependencies;
...
\end{enumerate}
For fixed \(\theta\), Bayes' rule gives
\[
  p_\theta
  =\Pr_\theta(H\mid E)
  =
