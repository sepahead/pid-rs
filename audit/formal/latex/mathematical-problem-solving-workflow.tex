...
% Keep the actor-attribution table's lead-in on one deterministic page boundary.  TeX Live 2024
% and the Ubuntu runner otherwise agree on the destination's page and type but can resolve the
% preceding stretchable space 33.098pt apart.  The exact-title guard scopes the forced boundary to
% this one report paragraph; the canonical Markdown and every other level-four heading are
% unchanged.
\def\PidWorkflowPortableParagraphTitle{Discovery, verification, and communication methods}
