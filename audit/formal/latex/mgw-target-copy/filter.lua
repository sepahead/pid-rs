...
function Para(e)
  -- Keep these exact short introductions with the display that follows.
  local text = pandoc.utils.stringify(e)
  if text == "write" or text == "while" then
...
function Table(e)
  ...
  local formal_map = n == 2 and #e.head.rows == 1
    and #e.head.rows[1].cells == 2
    and pandoc.utils.stringify(e.head.rows[1].cells[1].contents) == "targets"
    and pandoc.utils.stringify(e.head.rows[1].cells[2].contents) == "mathematical role"
  local latex = pandoc.write(pandoc.Pandoc({e}), "latex", PANDOC_WRITER_OPTIONS)
  if not formal_map then latex = latex:gsub("\\\\\n", "\\\\*\n") end
