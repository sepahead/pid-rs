...
[`lean-4.33.0-darwin-aarch64-current-project-replay-2026-08-11.json`](../evidence/lean-4.33.0-darwin-aarch64-current-project-replay-2026-08-11.json).
...
The versioned receipt records the official Darwin archive size and digest, executable identity,
...
`-I -S -B` (and `-O -I -S -B` for its optimized pair). The command environment is built from an
exact eight-variable nonsecret allowlist with fixed locale/timezone and fresh empty `HOME` and
...
The retained, exact-hash-bound runner is invoked with an explicit repository root, release-bin
directory, Python executable, archive, and output receipt:

```text
PID_LEAN433_BIN=/absolute/path/to/lean-4.33.0-darwin_aarch64/bin
PID_LEAN433_ARCHIVE=/absolute/path/to/lean-4.33.0-darwin_aarch64.tar.zst
python3 -I -S -B scripts/generate-lean-4.33-replay.py \
  "$(pwd -P)" "$PID_LEAN433_BIN" "$(command -v python3)" \
  "$PID_LEAN433_ARCHIVE" \
  audit/evidence/lean-4.33.0-darwin-aarch64-current-project-replay-2026-08-11.json
```

The runner opens the archive without following symlinks, requires a single-link regular file, and
checks stable descriptor identity, exact size, and SHA-256 before any replay command. Immediately
before the clean build it uses symlink-aware entry checks for absent project build/config paths and
the retained dependency-package directory.
...
that were stable at both replay endpoints. Finalization changes only the checker's one projection
literal; the final checker reconstructs the zero-placeholder replay bytes and verifies their hash,
while the self-test replay and final hashes must remain identical.
