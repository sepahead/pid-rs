...
[`lean-4.33.0-darwin-aarch64-current-project-replay-2026-08-18-r12.json`](../evidence/lean-4.33.0-darwin-aarch64-current-project-replay-2026-08-18-r12.json).
...
runner; none is current runner custody. The `r12` suffix denotes only the twelfth receipt in the
...
composite-v7 cut state from source bytes. The replay-projection line is exactly the unique zero
placeholder. The Lean composite-v7 scalar and its v7 operational-map row are identical nonzero
literals equal to the final composite-v7 checker SHA-256. That v7 checker contains one nonzero
`EXPECTED_NORMALIZED_LEAN_CHECKER_SHA256` literal equal to the Lean checker hash after normalizing
exactly the projection, v7 scalar, and v7 operational row to their exact placeholder forms. The
immutable composite-v4/v5/v6 checkers and r9/r10/r11 receipts remain prior evidence; none is
rewritten, and the ordinary retained v4/v5/v6 operational rows are outside this normalization.

The cut is finalized in one direction. Freeze the v7 self-test and every non-cut Lean input first;
compute `H_L` from the three-placeholder Lean normalization and place only `H_L` in the v7
checker; hash those final v7 checker bytes as `H_V`; place the same `H_V` in only the Lean v7 scalar
and v7 operational row; and keep the replay projection as the zero expression until the one-shot
generator succeeds. Changing the v7 self-test, documentation, or any other normalized Lean input
