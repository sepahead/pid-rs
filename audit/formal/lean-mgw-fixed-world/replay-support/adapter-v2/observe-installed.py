...
for i, pin in enumerate(pins):
    ...
    if i%10000==0:
        require(time.monotonic_ns()<reg['execution_end_monotonic_ns'],'original execution clock ended')
result={'utc':dt.datetime.now(dt.timezone.utc).isoformat(),'monotonic_ns':time.monotonic_ns(),
        ...
        'scope':'Sequential full installed-byte pre-observation for this separately admitted compliant-candidate production campaign; four Lake configuration cache files remain excluded. No atomic or loader-trace claim.'}
out=stage/'preparation/INSTALLED_PRE.json'
