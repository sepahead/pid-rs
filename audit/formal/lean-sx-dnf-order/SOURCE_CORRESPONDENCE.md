# Source correspondence and thirteen verified statements

This table accompanies [the exposition](EXPOSITION.md). All thirteen declarations passed the
complete semantic, axiom, and fresh-kernel judge in both Python modes for the accepted source
and its declared comment-only variant. The source mapping is an author/model review.
No named human disposition is recorded; Lean does not interpret the cited papers.
...
layout. The [retrieval record](../../evidence/dnf-order-formal-verification-2026-09-05/source-retrieval.json)
identifies the reviewed source bytes. Third-party PDFs remain local reference evidence and are
...
The accepted source's namespace is `PidSxDnfOrderCandidate`. It imports exactly `Contract`.
...
The existing source is [SxEventBridge.lean](../lean/PidFiniteConvergence/SxEventBridge.lean).
...
|---|---|
| Named source revisions to displayed definitions and orientation | Author/model source mapping with exact locators; no named human disposition is recorded. |
| Displayed definitions to existing event specification | Explicit mathematical and type-level map above; accepted theorem C1 connects the two formal representations. |
| Candidate declarations to frozen thirteen receiver-free targets | All thirteen complete target types and actual axiom sets passed the exact judge in four runs. |
| Fresh source closure checked by the separate kernel route | Fresh `leanchecker --fresh SemanticJudge` passed in each of the four accepted runs. The three earlier failures received no such credit. |
...
The exact [accepted source](Candidate.lean) SHA-256 is
...
These identities locate the exact source and contract. The [four-run record](../../evidence/dnf-order-formal-verification-2026-09-05/RESULTS.md)
supplies the scoped acceptance evidence. No mathematical novelty or external custody is claimed.

The adopted candidate policy permits only the transitive axiom subset
`propext`, `Classical.choice`, and `Quot.sound`. The actual records report only `propext` for `singleton_not_all_patterns` and all three allowed
axioms for the other twelve exports. The four actual record streams match byte for byte. No new axiom, `sorry`,
