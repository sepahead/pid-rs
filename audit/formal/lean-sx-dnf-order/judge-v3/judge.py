#!/usr/bin/env python3
"""Local revision-3 DNF/order judge; mathematical contract revision 2. It adds no claim of external judge custody."""
...
FREEZE_STATUS = 'frozen_after_candidate_before_replay'
PREDECESSOR_FREEZE_SHA256 = 'e4baedd90547d230e18bcfee1867d54e9697238f44ff306cbbd2529bc34b1d05'
AMENDMENT_CANDIDATE_SHA256 = '76298a9baaad74f831b5254ccdeb5eeda019ac67769df9fabf2a317694fc4a65'
...
def check_freeze(snapshot: Snapshot, expected: str) -> dict[str, object]:
    ...
    require(type(record) is dict and record.get('status') == FREEZE_STATUS
            and record.get('schema') == 'local-dnf-order-judge-freeze-v3',
            'judge freeze is not accepted for a candidate')
    require(record.get('candidate_exists') is True
            and record.get('predecessor_freeze_sha256') == PREDECESSOR_FREEZE_SHA256
            and record.get('candidate_sha256_at_amendment') == AMENDMENT_CANDIDATE_SHA256,
            'judge amendment chronology or predecessor binding changed')
    ...
    required = {name + '.lean' for name in MODULES} | {'judge.py', 'selftest.py', 'theorem-roster.json',
        'reporting-regression.py', 'AMENDMENT.md'}
