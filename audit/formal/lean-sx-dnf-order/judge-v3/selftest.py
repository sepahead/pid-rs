#!/usr/bin/env python3
"""Revision-3 judge controls after the candidate existed. This command supplies no candidate proof."""
...
def main():
    ...
    runner = None
    report = {'status': 'not_accepted', 'scope': 'Amendment controls after the candidate existed. Complete candidate replay is separate.',
    ...
    try:
        ...
        reporting = b'''import Lean
open Lean Elab Command
run_cmd liftTermElabM do
  let result := Lean.Json.mkObj [("status", "smoke-only")]
  IO.println ("DNF_REPORTING_SMOKE " ++ result.compress)
  pure ()
'''
        reporting_path = src / 'ReportingSmoke.lean'
        reporting_path.write_bytes(reporting)
        snapshots.read(reporting_path)
        emitted = runner.run([str(lean), '-t', '0', '-M', '4096', '-o',
            str(build / 'ReportingSmoke.olean'), str(reporting_path)], src,
            'reporting-smoke', allow_stdout=True)
        j.require(emitted == b'DNF_REPORTING_SMOKE {"status":"smoke-only"}\n',
                  'actual TermElabM reporting bytes changed')
        passed('term_elaboration_reporting_positive', 'reporting_regression',
               'Actual IO action in TermElabM; smoke data are not theorem evidence.')
        restored = reporting.replace(b'  IO.println', b'  liftIO <| IO.println')
        compile_module('ReportingRestoredLift', restored, 'CommandElabM Unit')
        restored_stdout = (output / f'{len(runner.commands)-1:02d}-ReportingRestoredLift' / 'stdout.log').read_bytes()
        j.require(b'TermElabM Unit' in restored_stdout and b'Type mismatch' in restored_stdout,
                  'restored lift did not fail at the original monad boundary')
        results[-1]['category'] = 'reporting_regression'
        results[-1]['details'] = 'Original CommandElabM lift rejected inside TermElabM.'
...
''', 'DNF_JUDGE_EXPORT_ROSTER')
        ...
        passed('P12_cosmetic_source_fragment', 'source_policy', 'Source syntax only; complete candidate semantic replay is separate.')
        mutations = {
        ...
        freeze_files.update({name:j.digest((LANE/name).read_bytes()) for name in ('judge.py','selftest.py','theorem-roster.json','reporting-regression.py','AMENDMENT.md')})
        frozenir=custody/'freeze';frozenir.mkdir()
        for name in freeze_files:(frozenir/name).write_bytes((LANE/name).read_bytes())
        frozen={'status':j.FREEZE_STATUS,'schema':'local-dnf-order-judge-freeze-v3',
                'candidate_exists':True,'predecessor_freeze_sha256':j.PREDECESSOR_FREEZE_SHA256,
                'candidate_sha256_at_amendment':j.AMENDMENT_CANDIDATE_SHA256,'files':freeze_files}
        ...
        try:
            ...
            for field, replacement, cause in (
                ('status', 'frozen_before_candidate', 'judge freeze is not accepted'),
                ('predecessor_freeze_sha256', '0'*64, 'judge amendment chronology'),
                ('candidate_sha256_at_amendment', '0'*64, 'judge amendment chronology')):
                changed = {**frozen, field: replacement}
                changed_raw = j.canonical(changed)
                (frozenir/'frozen.json').write_bytes(changed_raw)
                rejection('amendment_'+field, 'custody',
                    lambda changed_raw=changed_raw: j.check_freeze(j.Snapshot(),j.digest(changed_raw)), cause)
            (frozenir/'frozen.json').write_bytes(frozenraw)
        ...
        compile_module('ControlClosure', b'import ContractJudge\nimport FiniteControls\nimport WrongTargets\nimport PositiveTypeControls\nimport ReportingSmoke\n')
        runner.run([str(checker), '--fresh', 'ControlClosure'], src, 'kernel-all-positive-controls')
        passed('fresh_positive_control_closure', 'kernel')
        snapshots.verify()
        report['status']='amendment_controls_pass'
        report['pending']=['Complete thirteen-export positive, cosmetic and restored-reporting-module replay are separate obligations; the candidate existed before this amendment.',
    ...
    return 0 if report['status']=='amendment_controls_pass' else 1
