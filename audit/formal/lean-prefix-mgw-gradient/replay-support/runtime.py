#!/usr/bin/env python3
"""Local finite join/log judge, prepared before mathematical candidates. It adds no claim of external judge custody."""
...
CONTRACT_SHA256 = '100aabed35ab534e90306797f31003e0e2ae4c8f826687fc71f7731473129383'
THEOREMS = ('lower_cumulative_unique', 'join_convolution_cumulative', 'join_power_nonnegative', 'join_power_total', 'log_atoms_cumulative', 'supplied_log_inverse_nonnegative', 'log_atoms_strict_support', 'log_atoms_truncation')
MODULES = ('Contract', 'ClosedTargets', 'JudgeCore', 'JudgeTargets', 'JudgeAliases', 'ContractJudge', 'SemanticJudge')
ALLOWED_AXIOMS = frozenset({'Classical.choice', 'Quot.sound', 'propext'})
TIMEOUT_SECONDS = 600
MEMORY_BYTES = 4 * 1024**3
OUTPUT_BYTES = 4 * 1024**2
HEX = re.compile(r'[0-9a-f]{64}\Z')
FREEZE_STATUS = 'frozen_before_candidate_after_controls'
FREEZE_SCHEMA = 'local-join-log-judge-freeze-v1'
FROZEN_FILES = {name + '.lean' for name in MODULES} | {
    'judge.py', 'selftest.py', 'theorem-roster.json', 'MathematicalControls.lean',
    'MiniatureContract.lean', 'MiniatureCandidate.lean', 'MiniatureSemanticJudge.lean',
    'TypeControls.lean', 'PREPARATION_REGISTRATION.json',
    'RESOURCE_AND_EXECUTION_CONTRACT.md', 'MUTATION_ROSTER.json'}
...
def check_candidate_hash(raw: bytes, expected: object) -> None:
    require(type(expected) is str and HEX.fullmatch(expected) is not None
            and digest(raw) == expected, 'candidate attempt digest mismatch')
...
def candidate_policy(raw: bytes, base: ModuleType, path: Path) -> None:
    ...
    require(re.findall(r'(?m)^\s*namespace\s+([^\n]+)$', masked)
            == ['PidJoinLogCandidate'], 'candidate namespace roster changed')
    require(re.findall(r'(?m)^\s*end\s+([^\n]+)$', masked)
            == ['PidJoinLogCandidate'], 'candidate namespace closing changed')
    ...
    require(re.findall(r'(?m)^\s*universe\s+([^\n]+)$', masked) == ['u'],
            'candidate universe command must be exactly universe u')
    openings = re.findall(r'(?m)^\s*open\s+([^\n]+)$', masked)
    require(openings in ([], ['scoped BigOperators'], ['PidJoinLogContract'],
                        ['scoped BigOperators', 'PidJoinLogContract']),
            'candidate open commands are outside the fixed grammar')
...
def strict_axiom_output(raw: bytes) -> list[dict[str, object]]:
    ...
    for theorem, line in zip(THEOREMS, lines, strict=True):
        ...
        require(record['theorem'] == 'PidJoinLogCandidate.' + theorem
                and record['raw_target'] == 'PidJoinLogJudgeTargets.' + theorem
                and record['alias_target'] == 'PidJoinLogJudgeAliases.' + theorem
...
def register_child(command: list[str], cwd: Path, directory: Path, preparation: bool) -> dict[str, object]:
    record = {'registered_utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
              'command': command, 'cwd': str(cwd), 'output': str(directory),
              'preparation': preparation,
              'source_inputs': {str(p.relative_to(cwd)): digest(p.read_bytes())
                                for p in sorted(cwd.rglob('*.lean'))}}
    if preparation and Path(command[0]).name in {'lean', 'leanchecker'}:
        import fcntl
        path = LANE / 'CHILD_LEDGER.json'
        with path.open('r+', encoding='utf-8') as stream:
            fcntl.flock(stream.fileno(), fcntl.LOCK_EX)
            ledger = strict_json(stream.read().encode())
            task = strict_json((LANE / 'PREPARATION_REGISTRATION.json').read_bytes())
            require(datetime.datetime.now(datetime.timezone.utc)
                    < datetime.datetime.fromisoformat(task['deadline_utc']),
                    'judge preparation deadline exhausted')
            require(len(ledger['children']) < 96, 'judge preparation child budget exhausted')
            record['preparation_child'] = len(ledger['children']) + 1
            ledger['children'].append(record)
            stream.seek(0); stream.write(canonical(ledger).decode()); stream.truncate(); stream.flush()
...
class Runner:
    def __init__(self, output: Path, environment: dict[str, str], *, preparation: bool = False) -> None:
    ...
    def run(self, command: list[str], cwd: Path, label: str, *, timeout: float = TIMEOUT_SECONDS,
        ...
        directory.mkdir()
        registration = register_child(command, cwd, directory, self.preparation)
        ...
        try:
            ...
            if process is not None:
                try:
                    # Never signal a group after reaping its leader. A not-yet-reaped
                    # leader retains its numeric PID, including while it is a zombie.
                    if failure and process.returncode is None:
                        os.killpg(process.pid, signal.SIGKILL)
                except ProcessLookupError:
                    pass
                except BaseException as error:
                    failure, spawn_error = 'cleanup_signal_error', str(error)
                try:
                    process.wait(timeout=10)
                    cleanup_remaining = group_processes(process.pid)
                    if cleanup_remaining:
                        failure = 'postterminal_group_observed_without_identity_or_signal'
                except BaseException as error:
                    failure, spawn_error = 'cleanup_observation_error', str(error)
            for key in list(selector.get_map().values()):
                try:
                    key.fileobj.close()
                except BaseException as error:
                    failure, spawn_error = 'stream_cleanup_error', str(error)
            try:
                selector.close()
            except BaseException as error:
                failure, spawn_error = 'selector_cleanup_error', str(error)
...
def prepare(root: Path, output: Path, snapshot: Snapshot, *, preparation: bool = False) -> tuple[Runner, Path, Path, object]:
    ...
    runner = Runner(output, cleaned, preparation=preparation)
    ...
    for module in ('Contract', 'ClosedTargets', 'JudgeCore', 'JudgeTargets', 'JudgeAliases', 'ContractJudge'):
...
def check_freeze(snapshot: Snapshot, expected: str) -> dict[str, object]:
    require(type(expected) is str and HEX.fullmatch(expected) is not None, 'freeze digest must be SHA-256')
    raw = snapshot.read(LANE / 'frozen.json')
    require(digest(raw) == expected, 'adopted judge freeze digest changed')
    record = strict_json(raw)
    require(type(record) is dict and record.get('status') == FREEZE_STATUS
            and record.get('schema') == FREEZE_SCHEMA,
            'judge freeze is not accepted for a candidate')
    require(record.get('candidate_exists') is False
            and record.get('contract_sha256') == CONTRACT_SHA256,
            'judge chronology or contract binding changed')
    files = record.get('files')
    require(type(files) is dict and set(files) == FROZEN_FILES,
            'frozen judge file roster differs from exact policy')
    for name, expected_sha in files.items():
        require(type(expected_sha) is str and HEX.fullmatch(expected_sha) is not None
                and digest(snapshot.read(LANE / name)) == expected_sha,
                'frozen judge input changed: ' + name)
    return record
...
def main() -> int:
    ...
    try:
        ...
        check_candidate_hash(candidate_raw, args.candidate_sha256)
        ...
        snapshot.verify()
        receipt['candidate_sha256'] = digest(candidate_raw)
