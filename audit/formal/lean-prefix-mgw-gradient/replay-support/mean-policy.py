...
THEOREMS = ('word_coefficient_join_power', 'finite_block_expectation', 'prefix_expectations_to_mgw_mean')
IMPORTS = ("PidPrefixMgwMean.Contract", "PidMgwBridge.Candidate", "PidPrefixProbability.Candidate")
...
def candidate_policy(raw, base, path, *, final_control=False, development_helpers=False):
    ...
    require(re.findall(r"(?m)^\s*namespace\s+([^\n]+)$", masked)
            == ["PidPrefixMgwMeanCandidate"], "candidate namespace differs")
    require(re.findall(r"(?m)^\s*end\s+([^\n]+)$", masked)
            == ["PidPrefixMgwMeanCandidate"], "candidate namespace closing differs")
    ...
    require(len(openings) == len(set(openings))
            and set(openings) <= {"scoped BigOperators", "scoped BigOperators ENNReal", "scoped BigOperators ENNReal Topology", "MeasureTheory", "ProbabilityTheory", "PidPrefixMgwMeanContract", "PidMgwBridgeCandidate", "PidJoinLogContract", "PidJoinLogCandidate", "PidPrefixProbabilityContract", "PidPrefixProbabilityCandidate", "PidMgwBridgeContract"},
    ...
    private_names = set()
    saw_public = False
    closing = re.search(r"(?m)^\s*end\s+PidPrefixMgwMeanCandidate\s*$", masked)
    ...
    for index, marker in enumerate(markers):
        ...
        if final_control and name == THEOREMS[-1]:
            require(chunk == "theorem prefix_expectations_to_mgw_mean : True := by trivial",
...
def semantic_records(raw, strict_json):
    ...
    for index, (name, line) in enumerate(zip(THEOREMS, lines, strict=True)):
        prefix = "PREFIX_MGW_MEAN_JUDGE_RESULT "
        ...
        require(record["theorem"] == "PidPrefixMgwMeanCandidate." + name
                and record["raw_target"] == "PidPrefixMgwMeanRawTargets." + name
                and record["alias_target"] == "PidPrefixMgwMeanAliasTargets." + name
