...
THEOREMS = ('full_inverse_row', 'scalar_remainder_calculus', 'native_cumulative_increments', 'actual_atom_remainder', 'finite_block_score_derivative', 'actual_atom_score_bias', 'finite_latent_pushforward', 'finite_latent_score_derivative', 'finite_encoder_score', 'score_second_moment', 'latent_score_second_moment')
IMPORTS = ("PidPrefixMgwGradient.Contract",)
...
def candidate_policy(raw, base, path, *, final_control=False, development_helpers=False):
    ...
    require(re.findall(r"(?m)^\s*namespace\s+([^\n]+)$", masked)
            == ["PidPrefixMgwGradientCandidate"], "candidate namespace differs")
    require(re.findall(r"(?m)^\s*end\s+([^\n]+)$", masked)
            == ["PidPrefixMgwGradientCandidate"], "candidate namespace closing differs")
    ...
    private_names = set()
    saw_public = False
    closing = re.search(r"(?m)^\s*end\s+PidPrefixMgwGradientCandidate\s*$", masked)
    ...
    for index, marker in enumerate(markers):
        ...
        if final_control and name == THEOREMS[-1]:
            require(chunk == "theorem latent_score_second_moment : True := by trivial",
...
def semantic_records(raw, strict_json):
    ...
    for index, (name, line) in enumerate(zip(THEOREMS, lines, strict=True)):
        prefix = "PREFIX_MGW_GRADIENT_JUDGE_RESULT "
        ...
        require(record["theorem"] == "PidPrefixMgwGradientCandidate." + name
                and record["raw_target"] == "PidPrefixMgwGradientRawTargets." + name
                and record["alias_target"] == "PidPrefixMgwGradientAliasTargets." + name
                and record["status"] == "accepted", "semantic record identity differs")
        levels = record["universes"]
        require(type(levels) is list and len(levels) == (1, 0, 3, 3, 3, 3, 4, 4, 3, 3, 4)[index]
