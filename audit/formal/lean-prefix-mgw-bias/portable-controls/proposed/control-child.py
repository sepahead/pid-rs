...
def main():
    ...
    def patch_runtime(j):
        ...
        bias = json.loads((package / 'replay-support/EXPECTED_BIAS_RECORDS.json').read_bytes())
        ...
        class InertRunner:
            ...
            def run(self, command, cwd, label, *, timeout, allow_stdout=False):
                ...
                if case == 'dispatch-six-as-bias' and label == 'compile-PidPrefixMgwBias-SemanticJudge':
                ...
                if case == 'bias-reordered' and label == 'compile-PidPrefixMgwBias-SemanticJudge':
                    raw = encode(list(reversed(bias)), 'PREFIX_MGW_BIAS_JUDGE_RESULT ')
                if case == 'bias-type-drift' and label == 'compile-PidPrefixMgwBias-SemanticJudge':
                    values = json.loads(json.dumps(bias)); values[0]['full_elaborated_type'] += ' synthetic drift'
                    raw = encode(values, 'PREFIX_MGW_BIAS_JUDGE_RESULT ')
                if case == 'dispatch-three-as-bias' and label == 'compile-PidPrefixMgwBias-SemanticJudge':
                    raw = streams['compile-PidPrefixMgwMean-SemanticJudge']
                if case == 'dispatch-five-as-mean' and label == 'compile-PidPrefixMgwMean-SemanticJudge':
                    raw = streams['compile-PidPrefixMgwBias-SemanticJudge']
                if case in {'bias-fiber-universe-three', 'bias-other-universe-two', 'bias-axiom-drift',
                            'bias-raw-target-drift', 'bias-alias-target-drift'} and label == 'compile-PidPrefixMgwBias-SemanticJudge':
                    values = json.loads(json.dumps(bias))
                    if case == 'bias-fiber-universe-three': values[1]['universes'] = ['u', 'v', 'w']
                    if case == 'bias-other-universe-two': values[0]['universes'] = ['u', 'v']
                    if case == 'bias-axiom-drift': values[0]['axioms'] = ['sorryAx']
                    if case == 'bias-raw-target-drift': values[0]['raw_target'] += '_wrong'
                    if case == 'bias-alias-target-drift': values[0]['alias_target'] += '_wrong'
                    raw = encode(values, 'PREFIX_MGW_BIAS_JUDGE_RESULT ')
                ...
                wrong = job['action'] == 'wrong-last-target' and label == 'compile-PidPrefixMgwBias-SemanticJudge'
                if wrong and case != 'wrong-final-accepted':
                    raw = encode(bias[:4], 'PREFIX_MGW_BIAS_JUDGE_RESULT ')
                    raw += b'DNF_JUDGE_TARGET_TYPE type mismatch PidPrefixMgwBiasRawTargets.mass_floor_bias PidPrefixMgwBiasAliasTargets.mass_floor_bias\n'
                    code = 1
    ...
    def trace(frame, kind, value):
        ...
        if frame.f_code.co_filename == str(adapter):
            ...
            if kind == 'return' and value is not None and frame.f_code.co_name == 'load_module' and frame.f_locals.get('name') == 'accepted_bias_runtime':
