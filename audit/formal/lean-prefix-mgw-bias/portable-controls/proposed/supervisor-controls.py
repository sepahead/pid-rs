...
def main():
    ...
    require(registration['schema'] == 'portable-bias-supervisor-controls-execution-v1'
            and registration['status'] == 'root-reviewed-source-ready-for-controls'
            and type(registration['new_bias_theorems_proved']) is int
            and registration['new_bias_theorems_proved'] == 0, 'supervisor controls lack reviewed registration')
    ...
    result = {'status': 'failed', 'new_bias_theorems_proved': 0,
              'scope': 'actual supervisor and unchanged Runtime against bounded inert Python processes', 'records': []}
    try:
        for mode in (0, 1):
            # Admission/parser controls stop before supervise/Popen. The stale
            # mean adapter is inert inspection text, never a compiler route.
            for malformed, diagnostic in (
                    ('duplicate-plan-key', 'duplicate JSON key'),
                    ('nonfinite-plan', 'nonfinite JSON'),
                    ('duplicate-registration-key', 'duplicate JSON key'),
                    ('boolean-credit', 'supervisor controls carry no theorem credit'),
                    ('float-credit', 'supervisor controls carry no theorem credit'),
                    ('stale-mean-production-binding', 'reviewed adapter or manifest differs')):
                require(cutoff - time.monotonic() > 10, 'insufficient parser-control interval')
                directory = output / (str(mode) + '-' + malformed); directory.mkdir()
                stage = directory / 'stage'; stage.mkdir()
                born = dt.datetime.now(dt.timezone.utc)
                production = malformed == 'stale-mean-production-binding'
                task = {'registered_utc': born.isoformat(),
                        'deadline_utc': (born + dt.timedelta(minutes=120) if production else born + dt.timedelta(seconds=30)).isoformat(),
                        'owner': 'inert admission control'}
                write(stage / 'REGISTRATION.json', task)
                if malformed == 'duplicate-registration-key':
                    path = stage / 'REGISTRATION.json'
                    path.write_bytes(path.read_bytes().replace(b'{', b'{"owner":"duplicate",', 1))
                plan = {'schema': 'portable-bias-supervisor-plan-v1' if production else 'portable-bias-supervisor-control-plan-v1',
                        'status': 'root-reviewed-ready-to-execute', 'root': str(directory), 'stage': str(stage),
                        'fixture_mode': 'success', 'fixture_child': str(HERE / 'supervisor-child.py'),
                ...
                if malformed == 'boolean-credit': plan['new_bias_theorems_proved'] = False
                if malformed == 'float-credit': plan['new_bias_theorems_proved'] = 0.0
                if production:
                    predecessor = HERE.parent / 'inputs/binding-predecessor/replay-proposed.py.txt'
                    require(digest(predecessor.read_bytes()) == '121068917f2cdc0e3b3098c63869227be9511d7df5f9c011d3d68f980308220a',
                            'inspection-only mean predecessor changed')
                    plan['adapter'] = str(predecessor)
                write(directory / 'PLAN.json', plan)
                if malformed in {'duplicate-plan-key', 'nonfinite-plan'}:
                    path = directory / 'PLAN.json'
                    insertion = b'{"schema":"duplicate",' if malformed == 'duplicate-plan-key' else b'{"inert":NaN,'
                    path.write_bytes(path.read_bytes().replace(b'{', insertion, 1))
                ...
                with (directory / 'stdout.log').open('xb') as stdout, (directory / 'stderr.log').open('xb') as stderr:
                    terminal = subprocess.run(command, stdin=subprocess.DEVNULL, stdout=stdout, stderr=stderr,
                                              timeout=min(5, cutoff-time.monotonic()), check=False, env={'PATH': '/usr/bin:/bin'})
                require(terminal.returncode == 1 and diagnostic in (directory / 'stderr.log').read_text(),
                        malformed + ': causal admission refusal differs')
                require(not (directory / 'supervised').exists() and not (stage / 'FIXTURE_COMPLETED.json').exists(),
                        malformed + ': wrapper work began after admission refusal')
                result['records'].append({'mode': mode, 'case': malformed, 'status': 'matched_expected_behavior',
                                          'scope': 'admission refusal before supervisor wrapper spawn'})
            for name in CASES:
                ...
                write(stage / 'REGISTRATION.json', task)
                plan = {'schema': 'portable-bias-supervisor-control-plan-v1', 'status': 'root-reviewed-ready-to-execute',
