            ...
            if not path.exists():
                with path.open('xb') as stream:
                    stream.write(canonical({'reason': reason, 'observed_utc': utc().isoformat(),
                                            'owner': 'portable bias stage supervisor'}))
...
def main():
    ...
    require(plan['schema'] in {'portable-bias-supervisor-plan-v1', 'portable-bias-supervisor-control-plan-v1'}, 'plan schema differs')
    ...
    if controls:
        ...
        require(type(plan['new_bias_theorems_proved']) is int and plan['new_bias_theorems_proved'] == 0,
    ...
    try:
        for index, command in enumerate(commands):
            ...
            if not controls:
                transport = strict_json((output / ('wrapper-' + str(index).zfill(2)) / 'stdout.log').read_bytes())
                ...
                receipt = strict_json((attempt / 'RESULT.json').read_bytes())
