#!/usr/bin/env python3
"""Proposed portable bias replay; source-only, not execution-adopted.
...
SELECTED_SHA = 'c945862f2bb64825ad5b754cd6fcd4f32da02c0377f13074d9192b6568e3fb45'
COSMETIC_SHA = 'ac65476b7a101d344fff4fe0f4a9370ef7438b83e63d3854883e205c5ee33481'
BIAS_NAMES = ('actual_finite_bias',
 'fiber_inverse_moment',
 'support_moment_bounds',
 'projected_support_bias',
 'mass_floor_bias')
DEPS = ('PidFiniteConvergence/Deterministic',
...
TRUSTED = tuple('PidPrefixMgwBias/' + x for x in ('Contract', 'RawTargets', 'AliasTargets'))
MODULES = DEPS + TRUSTED + ('PidPrefixMgwBias/Candidate', 'PidPrefixMgwBias/SemanticJudge')
ROOT_MODULE = 'BiasCandidateRoot'
RUNTIME_PARENT = Path('.local/prefix-mgw-bias-public-replay-v1')
...
def main():
    ...
    require(manifest['schema'] == 'pid-rs/prefix-mgw-bias-portable-replay-proposal-v1', 'manifest schema differs')
    ...
    policies = {}
    for name in ('policy', 'prefix-policy', 'mgw-policy', 'mean-policy'):
        rel = 'replay-support/' + name + '.py'
        policies[name] = load_module(name.replace('-', '_'), HERE/rel, captured[rel], files[rel])
    policy = policies['policy']
    require(tuple(policy.THEOREMS) == BIAS_NAMES, 'bias theorem names changed')
    ...
    selected = captured['sources/PidPrefixMgwBias/Candidate.lean']
    ...
    require(registration['schema'] == 'pid-rs/prefix-mgw-bias-packaging-registration-v1'
    ...
    if not args.prepare_only:
        ...
        require(controls['schema'] == 'pid-rs/portable-bias-controls-acceptance-v1'
                and controls['status'] == 'adopted_completed_portable_bias_controls'
                and controls['manifest_sha256'] == args.manifest_sha256
                and controls['adapter_sha256'] == files['replay-proposed.py']
                and type(controls['new_bias_theorems_proved']) is int
                and controls['new_bias_theorems_proved'] == 0,
    ...
    require(sum(x['kind']=='compiler_kernel' for x in ledger['children']) < 186, 'child cap exhausted')
    ...
    result = {'schema':'pid-rs/prefix-mgw-bias-packaging-result-v1','status':'failed',
              'registered':entry,'scope':'Packaging replay after exact local acceptance; five bias exports, inherited eleven MGW, six prefix and three mean records.'}
    runner = None
    try:
        ...
        policy.candidate_policy(source,base,HERE/'sources/PidPrefixMgwBias/Candidate.lean')
        if args.action == 'wrong-last-target':
            ...
            for previous in ledger['attempts'][:-1]:
                ...
                if receipt_path.exists():
                    ...
                    if body['status']=='fresh_semantic_kernel_pass' and typed_equal(body['registered'],previous) and original==selected:
                        require(typed_equal(body['semantic_records'],expected_bias), 'prior semantic records changed')
            ...
            marker = 'theorem mass_floor_bias'
            text = selected.decode()
            masked = base.mask_lean_comments_and_strings(text,HERE/'sources/PidPrefixMgwBias/Candidate.lean')
            require(masked.count(marker)==1,'wrong-target marker differs')
            source = (text[:masked.index(marker)] + 'theorem mass_floor_bias : True := by trivial\n\nend PidPrefixMgwBiasCandidate\n').encode()
        ...
        for module in MODULES+(ROOT_MODULE,):
            raw = source if module=='PidPrefixMgwBias/Candidate' else captured['sources/'+module+'.lean']
        ...
        (output/'COMMAND_PLAN.json').write_bytes(canonical({'commands':commands,'probe':[str(lean),'--version'],
            'status':'prepared_no_execution','expected_compiler_kernel_children':31 if args.action=='check' else 29}))
        if args.prepare_only:
            ...
            def run(command,label,*,allow_stdout=False,kind='compiler_kernel'):
                ...
                cap = 6 if kind=='tool_probe' else 186
            ...
            for planned in commands:
                ...
                try:
                    ...
                    if args.action=='wrong-last-target' and module=='PidPrefixMgwBias/SemanticJudge':
                        ...
                        require(last['failure'] is None and last['returncode'] != 0
                                ...
                                and 'PidPrefixMgwBiasRawTargets.mass_floor_bias' in text
                                and 'PidPrefixMgwBiasAliasTargets.mass_floor_bias' in text
                        ...
                        prefix_text = 'PREFIX_MGW_BIAS_JUDGE_RESULT '
                        partial = [strict_json(line[len(prefix_text):].encode()) for line in text.splitlines()
                                   if line.startswith(prefix_text)]
                        require(typed_equal(partial,expected_bias[:4]),
                                'wrong-target control did not retain the first four exact accepted records')
                        result['status']='expected_target_rejection'
                        break
                    raise
                if module!='fresh-kernel':
                    snapshot.read(target)
                if module=='PidPrefixMgwBias/SemanticJudge':
                    records=policy.semantic_records(stdout,j.strict_json)
                    require(typed_equal(records,expected_bias),'exact accepted bias records differ')
...
if __name__=='__main__':
    try:
        ...
        print('Bias packaging replay refused: '+str(error),file=sys.stderr)
        raise SystemExit(1)
