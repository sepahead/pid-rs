...
| RB-02 | **partial** | Fragmentation is real. A tracked record cannot truthfully contain the SHA of the same commit that contains it without a cycle. Add a deterministic self-excluding source-scope manifest; produce commit-bound state as a post-commit CI/release artifact. Do not invent a circular `AUDIT_STATE.json`. |
