...
[`4.33.0 current replay receipt`](lean-4.33.0-darwin-aarch64-current-project-replay-2026-08-12-r4.json).
...
are preserved as prior evidence, not current runner custody. Here `r4` means only the fourth replay
receipt issued on 12 August UTC; it is not a schema, theorem, assurance-tier, or independence
revision. The route receives current execution credit only when the exact receipt exists and
validates.
