  ...
  calc
    ...
    _ = ∑ word : Fin (n + 1) → Key I X T, (∏ i, r (word i)) *
      ...
      have hne : List.ofFn word ≠ [] := by
        ...
        simp at hl
      apply congrArg (fun v : ℝ => (∏ i, r (word i)) * v)
      change cumulative (fun a : Node I =>
        if prefixEvent z a (List.ofFn word) then (1 : ℝ) else 0) b = _
      exact prefix_cumulative_indicator (I := I) (X := X) (T := T) z (List.ofFn word) hne b
