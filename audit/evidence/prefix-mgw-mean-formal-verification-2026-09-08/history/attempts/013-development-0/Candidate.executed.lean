...
private lemma mean_hasSum_of_raw_cumulative {I : Type u} [Fintype I] [Nonempty I]
  ...
  letI : Fintype (Node I) := nodeFintype I
  letI : SemilatticeSup (Node I) := sigma
  apply mean_hasSum_of_cumulative terms limit
  intro a
  have ha := h a
...
private lemma mean_word_cumulative
  ...
  have heq : (fun b => wordCoefficient r z b (n + 1)) =
      PidJoinLogContract.joinPower (weights r z) n := by
    funext b
    exact hword I X T r z b n sigma hsigma
  rw [heq, mean_cumulative_model sigma hsigma, mean_power_cumulative]
