# Local revision-2 judge execution contract
...
`leanchecker --fresh SemanticJudge`. The pre-candidate self-test uses one fresh kernel replay
...
## Resource revision adopted before the candidate
...
The adopted replacement uses the following controls:
...
```text
python3 -I -S -B .local/sx-dnf-order-judge-v2-20260905/selftest.py <root> <lane>/runs/<normal-output>
python3 -I -S -B -O .local/sx-dnf-order-judge-v2-20260905/selftest.py <root> <lane>/runs/<optimized-output>
python3 -I -S -B .local/sx-dnf-order-judge-v2-20260905/judge.py <root> <candidate> <lane>/runs/<attempt> --candidate-sha256 <candidate-sha256> --freeze-sha256 <adopted-frozen-json-sha256>
```

The coordinator must adopt the exact `frozen.json` digest before candidate work starts. The
candidate wrapper verifies that digest and every file that it binds. A changed source requires
...
The pre-candidate control result is deliberately narrower than candidate acceptance. No complete
thirteen-export candidate existed while the judge was prepared. The full positive candidate
and cosmetic semantic replays remain pending until a candidate exists. The finite controls are
