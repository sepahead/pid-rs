...
Updated: 2026-07-26. Active workstream: `KSG-INTEGER-HARMONIC-001`, revision 4 integration.
...
- Clean revision-4 candidate worktree for the current run:
  `/private/tmp/pid-rs-ksg-rev4.E11L9g/tree` on branch
  `codex/ksg-rev4-candidate-20260726`.
...
Before trusting a resumed run, reauthenticate these commits/trees and the frozen hashes in the
handoff. If a temporary candidate is absent, recreate it from the delivery parent and reconstruct
the reviewed changes; never substitute the ambient tree.
...
- [ ] Generate a canonical modular certificate for all 8,198 frozen rows.
- [ ] For each selected prime `1,000,033`, `1,000,037`, and `1,000,081`, independently replay 354
  structural zeros and 7,844 nonzero residues with the frozen ordered residue digest.
- [ ] Retain prime `1,000,003` and its four nonendpoint collisions as a negative control proving
  that residue zero does not imply rational zero in general.
- [ ] Use independent modular inverse/replay implementations where practical; record the shared
  formula/corpus cut and that the triple is fault diversity, not three proofs or CRT recovery.
- [ ] Mutate prime admissibility, primality, row order, endianness, residues, predicate, counts,
  implication direction, custody, schema, and canonicality.
- [ ] Author revision-4 claim, obligations, routes, witnesses, implementation map, correction
  ledger, failures, evidence matrix, decision, and revision index without changing frozen bytes.
- [ ] Add canonical `active-packet-v4.json`, exactly one active revision, regular-file/path checks,
  `--claim-only`, hash-first mutations, and one-hash-rebased semantic mutations.
- [ ] Explicitly bind the KSG/Ehrlich/PID2/MGW non-transfer firewall and revision 4's post-result
  status.
...
- [ ] Reconstruct parent-plus-KSG-only `stats.rs`, KSG/ISX/PID3 callers, tests, fixture, and
  generator; exclude exact-binary64-sum, PID2 represented-sum, and Imin changes.
- [ ] Bind W1 production radius `79`, ordered counts `(4,1)`, exact target `107/210`, and selected
  bits in brute and kd-tree paths.
- [ ] Bind W2 inclusive mapping and public propagation, using “eight ordered-binary64 positions.”
- [ ] Derive the 240/114 endpoint split in Python and Rust; bind 354 selected `+0`, zero selected
  `-0`, and 150/354 nonzeros for ordinary-left association over the selected Neumaier prefix.
- [ ] Recapture all 13 serial constants using parent PID2 code plus KSG-only changes; replay exact
  equality with `parallel` and thread-budget variations.
- [ ] Move catalog evidence to revision 4 and derive the exact 21-node reverse closure minus the
  sole shared-config exclusion: 20 affected and 49 protected method objects, 45 references, and
  protected metadata.
- [ ] Bind 15 affected and 20 protected full release-family objects; retain KSG-only bridge strings
...
- [ ] Add a candidate-tree phase checker: exact parent, exact path allowlist, four-path delivery
  ...
  hash-rebased semantic mutations.
- [ ] Regenerate `METHODS.md` and `RELEASE_SCOPE_1_0.md`; rebuild review evidence, dispositions,
  assurance registry, ecosystem bindings, and software identity only after all sources settle.
- [ ] Preserve ecosystem consumer/inventory objects; record the historical projection rather than
...
- [ ] Run generator, exact, Decimal, binary64, certificate, claim, formal, source, compiled
  ...
  candidate bytes.
- [ ] Obtain final source-blind and proof-blind native/Fable/Opus attacks; independently adjudicate
  every point and rerun affected gates.
...
- The current ambient main self-test reported 142 rejected mutations plus two scope preflights in
  ...
  absent, so this result cannot close revision 4.
- All three independent intake audits classify repository integration as NO-GO and prohibit an
  ambient commit.
