#!/usr/bin/env python3
"""Portable reference calculation for the fixed recorded-office OR study.

Author: Sepehr Mahmoudian. Historical outputs are preserved separately.
No tuning, raw-sensor reconstruction, population inference or PID advantage.
Operational execution limits belong to the separately admitted caller.
"""
import argparse
import hashlib
import json
...
EXPECTED_DATA_SHA256 = '15798710c848225b45276134fa26ae5c04f9ffb0535f2bea23e33a4e19cd3487'
EXPECTED_RUNTIME_SHA256 = '5f62b1de0c802fb14cd595a0a2afdfa04c27bb6c8084f0088a7228fb642380eb'
HISTORICAL_PROTOCOL_SHA256 = 'da493d9bdf3e82a7aa5fb88208827ef757fb67701a5d191736dc21b4ca2d5a55'
HISTORICAL_CALCULATOR_SHA256 = '72d17b31f3ba82afaded00a3382d85e655b08b854e148f2223c5be0ea7c0cceb'
...
def main():
    ...
    parser.add_argument('--data-dir', type=Path, required=True,
                        help='Directory containing the two exact published input JSON files')
    parser.add_argument('--output', type=Path,
                        help='Create this result JSON; otherwise write it to stdout')
    args = parser.parse_args()
    data_raw = (args.data_dir / 'descriptive-comparisons.json').read_bytes()
    runtime_raw = (args.data_dir / 'runtime-results.json').read_bytes()
    if hashlib.sha256(data_raw).hexdigest() != EXPECTED_DATA_SHA256:
        raise RuntimeError('Published descriptive-count input hash differs')
    if hashlib.sha256(runtime_raw).hexdigest() != EXPECTED_RUNTIME_SHA256:
        raise RuntimeError('Published Rust-result input hash differs')
    data = json.loads(data_raw)
    runtime = {r['recording']: r for r in json.loads(runtime_raw)['results']}
    ...
    out = {'schema': 'pid-rs/or-event-development-reproduction-v1',
           'author': 'Sepehr Mahmoudian',
           'historical_calculator_sha256': HISTORICAL_CALCULATOR_SHA256,
           'protocol_sha256': HISTORICAL_PROTOCOL_SHA256,
           'data_sha256': EXPECTED_DATA_SHA256,
           'runtime_sha256': EXPECTED_RUNTIME_SHA256, 'tolerance': TOL,
    ...
    encoded = json.dumps(out, indent=2, sort_keys=True, allow_nan=False) + '\n'
