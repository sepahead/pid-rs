...
payload, and label JSON. Summary and manifest sidecars generated by v0.5 add `trace_hash_v2` and
`logical_trace_hash_v3` fields for these generations; the old unversioned fields retain their
schema-1 values whenever those can be computed. If a number exceeds finite `f64`, the compatibility
field falls back to its lossless counterpart so summarization still succeeds. Sidecars written
before v0.5 remain readable and verifiable without the added fields.
