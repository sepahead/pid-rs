...
/// is returned as signed infinity.
#[inline]
...
fn round_product_sum(magnitude: &[u64; PRODUCT_SUM_LIMBS], negative: bool) -> f64 {
    let highest = highest_set_bit(magnitude).expect("nonzero exact product sum");
...
mod tests {
    ...
    #[test]
    fn lorentz_dot_returns_signed_infinity_only_for_an_uncancelled_overflow() {
        assert_eq!(
            lorentz_dot(&[f64::MAX, 0.0], &[f64::MAX, 0.0]),
            f64::NEG_INFINITY
        );
    }
    ...
    fn lorentz_superaccumulator_rounds_representative_single_products_like_binary64() {
        ...
        for &left in &values {
            for &right in &values {
                ...
                let actual = lorentz_dot(&[0.0, left], &[0.0, right]);
                assert_eq!(
                    actual.to_bits(),
                    expected.to_bits(),
