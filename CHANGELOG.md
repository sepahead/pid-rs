...
- Preserve five exact July 2026 advisory-prompt bytes in a non-authoritative, inert archive with a
  ...
  25 hash-only withheld companion observations, and ten candidates that remain
  `not_imported_requires_fresh_derivation`. The 15 added hash-only records account for 14
  Opus-family artifacts and one PID2 adversarial prompt without importing their bytes. A
  fail-closed checker and 64-mutation isolated hostile suite reject payload, provenance, status,
  successor, license/privacy, zero-byte/digest aliasing, withheld, authority-wiring,
  execution/import, symlink, schema-cardinality, and README-iff drift. This archive adds no
  scientific evidence, theorem, estimator, execution result, model-output validity, or release
  authority.
...
- Published composite-v8 C8 as exact commit `7c80d48db415279fc4d744eadb1515797606912b`
  ...
  then pointed to that commit, `main` remained exact C8, and GitHub exposed zero workflow runs for
  it. Its 145,356-byte same-slot `r14` has SHA-256
  ...
  identity only, not acceptance, attestation, or semantic-correctness evidence. The production
  local recorder failed closed and issued no L9. Its generic 51-byte stderr has SHA-256
  ...
  neither the failure stage nor the production command streams. In the exact rejected source
  route, `run_bounded` returns before `validate_record_value`, whose post-command record validation
  calls `validate_authority_roster`. That source ordering does not retain or authenticate the
  production stdout or stderr. Separate direct and sanitized operator invocations of
  `just ksg-composite-v9` both exited zero and produced identical 32,248-byte stdout with SHA-256
  ...
  named-roster classification defect, not authority-size exhaustion. The substituted diagnostic
  made one substituted call and then failed at that postcondition, diagnosing the defect within
  the substituted route. It is not production execution or custody, does not prove or retain the
  production command streams, and its call must not be relabelled as the recorder-owned production
  `just` invocation. The mutable ref is a recovery locator, not authentication or durability. No L9,
  hosted, C9/R9, scientific, accepted-current-replay, or independence credit transfers. Do not
  merge or cherry-pick the candidate or reuse its evidence. A fresh current `r14` must exclude all
...
- Published composite-v5 C5 as exact commit
  ...
  normal and optimized custody checks. This is one project-defined PDF-checker association repair,
  not a new PID definition,
...
- Withdrew unpublished local C5 candidate
  `08cf441c34b0d7a547d906ed682693c57ff29e25` after its fresh 135,997-byte `r10`
  (SHA-256 `68dceeff2e031339a985e91438c7e60c939da914171b90fb55a8472d006b350a`)
  passed Lean execution and receipt-finalization checks but the required
  `just ksg-composite-v5` route failed before push. The recipe's deliberate `umask 077` made the
  checkout-normalizer self-test create its supposed mode-0644 positive fixture as mode 0600; the
  ...
  failure surface or a change to the five bounded predecessor repairs. The accepted r10 slot
  remains available only to a fresh one-shot replay over the corrected final bytes; archived
  `08cf441` and its receipt remain outside `prior_replay_*` and are never copied, merged, or
  relabelled as accepted evidence.
